...
Typical usage:
    scripts/generate_model.py \
        models/mongla_heavy/model.sdf.in \
        models/mongla_heavy/model.sdf \
        models/mongla_heavy/configs.yaml
"""
...
# what the sim_front and sim_bottom profiles in mongla_vision/config/cameras.yaml
# declare, and mongla_ws is the consumer we must not force to change.
CAMERA_DEFAULTS = {
    "width": 640,
    "height": 480,
    "update_rate": 30.0,
...
DVL_DEFAULTS = {
    ...
    #
    # Nothing is lost by default: mongla_sim_bridge/dvl_bridge.py publishes an
...
class ModelParams:
    ...
    def __init__(
        ...
        # colour, which is what made Mongla_agile a pale blob.
        ...
        for i, thruster in enumerate(thrusters):
            ...
            # the template.
            #
            # The BlueROV2 template hard-codes -45/-135/+45/+135, and Mongla_agile
...
def _repaint_meshes(config_path: str, model_dir: str) -> None:
    ...
    # Only the BlueROV2-derived vehicle wears the generated livery. Mongla_agile's
    ...
    if not os.path.isfile(os.path.join(meshes, "mongla_heavy.dae")):
        return
    for stem in ("mongla_heavy", "t200_ccw_prop", "t200_cw_prop"):
