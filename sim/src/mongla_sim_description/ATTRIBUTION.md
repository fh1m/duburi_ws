...
`scripts/generate_model.py`, `models/mongla_heavy/model.sdf.in` and the
...
- Renamed the model to `mongla_heavy` and parameterised the Thruster
...
- Added front and bottom camera links, joints and sensors, sized to the
  `sim_front` / `sim_bottom` profiles that `mongla_ws` expects. Upstream ships
  no cameras at all.
...
- Populated the added-mass and linear-damping Fossen coefficients, which
  upstream leaves at zero. See `models/mongla_heavy/HYDRODYNAMICS.md`.
...
`meshes/mongla_heavy.dae`, `meshes/t200_ccw_prop.dae` and
...
See `models/mongla_heavy/HYDRODYNAMICS.md` for per-coefficient sourcing.
