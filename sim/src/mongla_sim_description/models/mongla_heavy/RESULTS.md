...
Measured by `mongla_sim_worlds/scripts/drag_survey.py` against
`/model/mongla/odometry` ground truth. A coefficient change should be diffed
against these rather than re-derived.

```bash
ros2 launch mongla_sim_bringup sim.launch.py course:=pool_empty \
