...
server = pytest.importorskip('mongla_sim_web.server')
from mongla_sim_bridge.paths import runtime_dir  # noqa: E402
...
INJECTIONS = [
    'arm; touch /tmp/mongla_pwned',
    'arm && id',
    'arm | tee /tmp/mongla_pwned',
    '$(id)',
    '`id`',
    'arm\nid',
    '../../../bin/sh',
...
def test_metacharacter_verbs_are_rejected(verb):
    with pytest.raises(Exception) as exc:
        server._mongla_cmd(verb)
    assert getattr(exc.value, 'status_code', None) == 400, exc.value


def test_a_real_verb_is_accepted_and_lands_in_argv():
    argv = server._mongla_cmd('move_forward', '--duration', '5')
    # ['bash', '-c', <script>, <mongla_ws>, 'move_forward', '--duration', '5']
...
# --------------------------------------------------------------------------
# 6. MONGLA_LAB_HOST: the shell message and the Python bind must agree
# --------------------------------------------------------------------------
#
...
#
#   shell   ${MONGLA_LAB_HOST:-127.0.0.1}          substitutes on unset OR EMPTY
...
# So `MONGLA_LAB_HOST= mongla_sim lab` printed "LOOPBACK ONLY" while the server
...
# `MONGLA_LAB_HOST=` line in .env, systemd `Environment=MONGLA_LAB_HOST=`.

def _lab_script():
    """Find scripts/mongla_sim by walking up, not by a parents[] index.
    ...
    """
    rel = Path('src') / 'mongla_sim_bringup' / 'scripts' / 'mongla_sim'
...
def _python_host(value):
    ...
    env = {} if value is None else {'MONGLA_LAB_HOST': value}
    return (env.get('MONGLA_LAB_HOST') or '127.0.0.1').strip() or '127.0.0.1'


def _shell_host(value):
    """Exactly the normalisation `mongla_sim lab` exports."""
    script = (
    ...
    import os
    env = dict(os.environ)
    env.pop('MONGLA_LAB_HOST', None)
    if value is not None:
        env['MONGLA_LAB_HOST'] = value
...
def test_shell_and_python_agree_on_the_bind_host(value):
    assert _shell_host(value) == _python_host(value), (
        f'MONGLA_LAB_HOST={value!r}: the CLI announces one host and the server '
        f'binds another'
    )
...
def test_the_cli_over_warns_rather_than_under_warns():
    ...
    assert 'export MONGLA_LAB_HOST="$_lab_host"' in s, \
