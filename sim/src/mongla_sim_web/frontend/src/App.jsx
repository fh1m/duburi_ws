...
// Deadzone + expo, identical maths to mongla_sim_web/joystick.py so a pad on
    ...
    <div className="joy-panel">
      ...
      {active ? (
        ...
        <div className="muted joy-hint">
          ...
          {' '}<code>MONGLA_JOYSTICK=/dev/input/js0</code>
        </div>
      )}
    </div>
  )
...
function linkDiagnosis(link = {}, teleop) {
  if (!link.gz) return ['gazebo is not running', 'mongla_sim sim']
  if (!link.sitl) return ['ArduSub SITL is not up', 'restart: mongla_sim sim']
  if (!link.mav) return ['no MAVLink from the manager', 'mongla_sim stack']
  if (!link.cams) return ['camera bridge has no frames', 'check mongla_sim sim log']
...
// Pool floor from mongla_sim_worlds/spec/arena.yaml (surface z=0, depth 1.6).
...
//
// Kept in sync with mongla_sim_worlds/spec/*.yaml by hand -- the lab has no
  ...
  return (
    <section className="page single">
      <div className="panel wide">
        ...
        <p className="muted">
          timeseries: <code>ros2 run mongla_sim_bringup mongla_sim plotjuggler</code>
        </p>
      ...
      <header>
        <div className="brand">
          ...
          <div>
            ...
            <div className="sub">mongla · mongla sim lab</div>
          </div>
        </div>
        <nav>
          {PAGES.map((p) => (
