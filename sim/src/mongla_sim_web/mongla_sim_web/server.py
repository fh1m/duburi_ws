...
"""FastAPI operator lab for the Mongla simulator."""
...
from mongla_sim_web.ros_bridge import RosWorker
from mongla_sim_web.script_runner import ScriptRunner
from mongla_sim_web.joystick import JoystickReader
from mongla_sim_web.teleop import TeleopStreamer
...
def _workspace_root() -> Path:
    # Shared with record_cameras via mongla_sim_bridge.paths -- see that module
    # for why the old name-match + cwd fallback was deleted. mongla_sim_web
    # already exec_depends on mongla_sim_bridge, so this is not a new edge.
    from mongla_sim_bridge.paths import sim_ws_root

    return sim_ws_root()


def _rt(name: str) -> Path:
    """A side-channel file under /tmp/mongla-$USER/ (see paths.runtime_dir)."""
    from mongla_sim_bridge.paths import runtime_dir

    return runtime_dir() / name


def _scripts_dir() -> Path:
    env = os.environ.get('MONGLA_SIM_SCRIPTS')
    if env:
        return Path(env)
    try:
        from ament_index_python.packages import get_package_share_directory

        return Path(get_package_share_directory('mongla_sim_web')) / 'scripts'
    except Exception:
        return PKG / 'scripts'
...
def _static_dir() -> Optional[Path]:
    ...
    try:
        ...
        candidates.insert(0, Path(get_package_share_directory('mongla_sim_web')) / 'static')
    except Exception:
        pass
    for c in candidates:
...
def _courses_dir() -> Path:
    try:
        ...
        return Path(get_package_share_directory('mongla_sim_worlds')) / 'courses'
    except Exception:
        return _workspace_root() / 'src' / 'mongla_sim_worlds' / 'courses'
...
joystick = JoystickReader(
    teleop,
    device=os.environ.get('MONGLA_JOYSTICK') or None,
...
app = FastAPI(title='Mongla Sim Lab', version='0.3.0')
...
def _manager_up() -> bool:
    return (
        subprocess.run(['pgrep', '-f', 'lib/mongla_manager/start'], capture_output=True).returncode
        == 0
    )
...
def _ensure_prop_manager(course: str) -> None:
    ...
    subprocess.run(
        ['pkill', '-f', 'lib/mongla_sim_scenarios/prop_manager'],
...
def _run_sim_bringup(course: str, gui: bool, with_stack: bool, do_stop: bool) -> None:
    ...
    try:
        ...
        sim_args = ['ros2', 'run', 'mongla_sim_bringup', 'mongla_sim', 'sim', f'course:={course}']
        if not gui:
...
def sim_stop():
    ...
    subprocess.run(['ros2', 'run', 'mongla_sim_bringup', 'mongla_sim', 'stop'], check=False)
...
def score():
    ...
    if not card:
        return {'available': False,
                'hint': 'no /mongla/sim/score -- start the sim with '
...
def _mongla_cmd(verb: str, *extra: str) -> list[str]:
    """Build a shell invocation that sources mongla_ws then runs the planner CLI.
    ...
    Deliberately NOT validated against mongla_control.commands.COMMANDS: that import
    is only available when mongla_ws happens to be on PYTHONPATH, so the check would
    ...
    """
    from mongla_sim_bridge.paths import mongla_ws_root

    if not _VERB_RE.match(verb):
        raise HTTPException(400, f'invalid verb {verb!r}')

    mongla_ws = str(mongla_ws_root())
    script = (
        'source /opt/ros/humble/setup.bash && '
        'test -f "$0/install/setup.bash" && source "$0/install/setup.bash"; '
        'exec ros2 run mongla_planner mongla "$@"'
    )
    return ['bash', '-c', script, mongla_ws, verb, *extra]
...
def vehicle_cmd(body: MoveBody):
    ...
    args = _mongla_cmd(body.cmd, *extra)
...
def props_catalog():
    try:
        from mongla_sim_scenarios.prop_catalog import load_prop_library, prop_anchor, prop_names
...
def props_spawn(body: PropBody):
    args = [
        'ros2', 'run', 'mongla_sim_scenarios', 'props', 'add',
...
def props_move(body: PropMoveBody):
    args = [
        'ros2', 'run', 'mongla_sim_scenarios', 'props', 'move',
...
def props_remove(name: str):
    proc = subprocess.run(
        ['ros2', 'run', 'mongla_sim_scenarios', 'props', 'remove', name],
        capture_output=True,
        text=True,
        timeout=60,
    )
...
def _models_dirs() -> list[Path]:
    dirs = []
    src = _workspace_root() / 'src' / 'mongla_sim_worlds' / 'models'
    if src.is_dir():
        dirs.append(src)
    try:
        from ament_index_python.packages import get_package_share_directory

        share = Path(get_package_share_directory('mongla_sim_worlds')) / 'models'
...
async def assets_upload(file: UploadFile = File(...)):
    """Upload a prop model zip containing model.sdf into mongla_sim_worlds/models."""
    if not file.filename or not file.filename.lower().endswith('.zip'):
        raise HTTPException(400, 'upload a .zip containing model.sdf (+ meshes)')
    dest_root = _workspace_root() / 'src' / 'mongla_sim_worlds' / 'models'
...
def main(argv=None) -> int:
    ...
    # MONGLA_LAB_HOST=0.0.0.0 is the explicit opt-in for a topside laptop.
    #
    # `or`, NOT os.environ.get(k, default): a SET-BUT-EMPTY var is a real case
    # (`MONGLA_LAB_HOST= mongla_sim lab`, an unset key in docker --env-file, a
    # bare `MONGLA_LAB_HOST=` in .env or systemd Environment=). get() returns ''
    ...
    host = (os.environ.get('MONGLA_LAB_HOST') or '127.0.0.1').strip() or '127.0.0.1'
    preferred = int(os.environ.get('MONGLA_LAB_PORT', '28765'))
    ...
    os.environ['MONGLA_LAB_PORT'] = str(port)
    try:
