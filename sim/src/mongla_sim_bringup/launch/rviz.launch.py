"""RViz2 for the Mongla simulator: robot model, sensors, and truth vs belief.
...
Brings up three things:
  ...
  mongla_sim_bridge/tf_broadcaster  odom -> base_link from ground truth
  rviz2                  with config/mongla.rviz

Run standalone against a live sim:
    ros2 launch mongla_sim_bringup rviz.launch.py
or as part of the sim:
    ros2 run mongla_sim_bringup mongla_sim sim rviz:=true
"""
import os
...
def generate_launch_description():
    description_share = get_package_share_directory('mongla_sim_description')
    bringup_share = get_package_share_directory('mongla_sim_bringup')

    urdf_path = os.path.join(description_share, 'urdf', 'mongla_heavy.urdf')
    rviz_config = os.path.join(bringup_share, 'config', 'mongla.rviz')
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            name='mongla_sim_robot_state_publisher',
            output='screen',
