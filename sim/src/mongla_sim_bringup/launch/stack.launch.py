"""Bring up the unmodified mongla_ws stack against a running simulator.

Prerequisites: the simulator is already up

    ros2 launch mongla_sim_bringup sim.launch.py course:=sauvc26_qualification gui:=false

Then, in a second terminal (both workspaces sourced):

    ros2 launch mongla_sim_bringup stack.launch.py
...
def generate_launch_description():
    ...
    payload_arg = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            f'/tmp/mongla-{os.environ.get("USER", "user")}', 'payload'),
    ...
    bno_arg = DeclareLaunchArgument(
        'bno085_port',
        default_value=os.path.join(
            f'/tmp/mongla-{os.environ.get("USER", "user")}', 'bno085'),
    ...
    args = [
        ...
        DeclareLaunchArgument(
            'bottom_image_topic',
            default_value='/mongla/sim/bottom_camera/image_raw',
        ...
        DeclareLaunchArgument(
            'image_topic',
            default_value='/mongla/sim/front_camera/image_raw',
            description='Gazebo front-camera topic from the sim bridge.',
        ),
    ]

    # Reuse mongla_manager's own bringup so we stay aligned with its defaults,
    ...
    # would name the detector /mongla_detector_sim_front while missions look for
    # /mongla_detector_forward.
    ...
    manager = GroupAction([IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory('mongla_manager'),
    ...
    # /mongla/vision/<name>/* by those names (AUDIT.md:74).
