"""Top-level entry point for the Mongla simulator.

    ros2 launch mongla_sim_bringup sim.launch.py course:=sauvc26_qualification
...
Then run the autonomy stack against it:
    ...
    source ~/Ros_workspaces/mongla_ws/install/setup.zsh
...
def generate_launch_description():
    bringup_share = get_package_share_directory('mongla_sim_bringup')
    worlds_share = get_package_share_directory('mongla_sim_worlds')
    ...
    args = [
        DeclareLaunchArgument(
            # MONGLA IS THE DEFAULT VEHICLE. This is the MODEL name, and it
            ...
            # vehicle simply sits there, and nothing logs a reason. `mongla` is
            # the INSTANCE name and is unchanged, which is what payload_sim,
            # scoring and bno085_sim address.
            'vehicle_model', default_value='mongla_heavy',
        ...
        DeclareLaunchArgument(
            ...
            description='Course world in mongla_sim_worlds/worlds. '
                        'Run `gen_world.py --list` to see the options.',
        ),
        DeclareLaunchArgument(
            'vehicle_name',
            default_value='mongla',
        ...
        DeclareLaunchArgument(
            'ardusub_params', default_value='mongla_sub.parm',
            description='Parameter overlay in mongla_sim_bringup/config. Use '
                        'mongla_sub_extnav.parm to test the ATT_POS_MOCAP path.',
        ),
    ...
    ardusub = ExecuteProcess(
        cmd=[
            ...
            # Primary MAVLink link out to mongla_ws.
    ...
    # alternative is mongla.fire() silently doing nothing in sim -- the exact
    # gap this node exists to close. Point the manager at it with
    # `payload_port:=/tmp/mongla-$USER/payload`.
    payload = Node(
        package='mongla_sim_bridge',
    ...
    bridge = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory('mongla_sim_bridge'),
                'launch',
                'bridge.launch.py',
            )
        ),
