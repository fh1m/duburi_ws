...
from mongla_sim_bridge.scoring import AxisStyle, _quadrant
...
def test_roll_and_pitch_outscore_yaw():
    ...
    from mongla_sim_bridge.scoring import (
...
def test_pose_stream_binary_is_in_the_stop_kill_list():
    """`mongla_sim stop` must kill what the scoring node spawns.
    ...
    stop = (root / 'mongla_sim_bringup' / 'scripts' / 'mongla_sim').read_text()
    assert 'gz-transport-topic' in stop, (
        'scoring spawns `gz topic -e`; add its binary to mongla_sim stop')
