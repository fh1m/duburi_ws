...
`mongla_sim_worlds/scripts/payload_check.py` against a running sim. These are
...
from mongla_sim_bridge.payload_sim import (
    DROPPER_CHANNELS,
    SHOT_SLOTS,
    TORPEDO_CHANNELS,
    _projectile_sdf,
...
def test_shot_slots_match_the_world_generator():
    ...
    gen = os.path.join(here, '..', '..', 'mongla_sim_worlds', 'scripts',
                       'gen_world.py')
    if not os.path.exists(gen):
        pytest.skip('mongla_sim_worlds sources not present')
    with open(gen) as f:
...
def test_channel_map_matches_the_real_driver():
    from mongla_control.payload import CHANNEL_NAMES
