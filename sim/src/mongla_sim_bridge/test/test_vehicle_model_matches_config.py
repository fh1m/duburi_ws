...
box -- and both `model.sdf` and `urdf/mongla_heavy.urdf` are derived from it by
...
DESC = os.path.abspath(os.path.join(
    os.path.dirname(__file__), '..', '..', 'mongla_sim_description'))
MODEL = os.path.join(DESC, 'models', 'mongla_heavy')
...
URDF = os.path.join(DESC, 'urdf', 'mongla_heavy.urdf')

pytestmark = pytest.mark.skipif(
    not os.path.isfile(CONFIG), reason='mongla_sim_description not present')
...
def test_the_committed_sdf_matches_configs_yaml():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _diff(out, SDF,
              'cd sim/src/mongla_sim_description && scripts/generate_model.py '
              'models/mongla_heavy/model.sdf.in models/mongla_heavy/model.sdf '
              'models/mongla_heavy/configs.yaml')


def test_the_committed_urdf_matches_configs_yaml():
    with tempfile.TemporaryDirectory() as tmp:
        out = os.path.join(tmp, 'mongla_heavy.urdf')
        _run('generate_urdf.py', CONFIG, out)
        _diff(out, URDF,
              'cd sim/src/mongla_sim_description && scripts/generate_urdf.py '
              'models/mongla_heavy/configs.yaml urdf/mongla_heavy.urdf')
