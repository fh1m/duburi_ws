...
ROS at all: mongla_ws speaks to the vehicle with direct pymavlink over
udpin:0.0.0.0:14550 and needs a specific set of messages at specific rates.

It connects the same way mongla_ws does and asks for the same message intervals,
so if this passes there is nothing left for the autonomy stack to trip over.

Usage:
    ros2 run mongla_sim_bridge mavlink_check
    ros2 run mongla_sim_bridge mavlink_check --timeout 90
...
# What mongla_manager's connection_config.py uses in sim mode.
...
def main() -> int:
    ...
    # Request the same intervals mongla_ws requests at startup.
