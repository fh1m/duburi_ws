...
Usage:
    ros2 run mongla_sim_bridge record_cameras --duration 60
    ros2 run mongla_sim_bridge record_cameras --fx --frames --labels --label gate_approach
"""
...
from mongla_sim_bridge.underwater_fx import apply_underwater_fx
from mongla_sim_bridge.box_labels import (
    CLASSES,
    boxes_to_yolo,
    format_yolo_line,
    write_classes_file,
...
FRONT_RAW = '/mongla/sim/front_camera/image_raw'
BOTTOM_RAW = '/mongla/sim/bottom_camera/image_raw'
FRONT_BOXES = '/mongla/sim/front_camera/boxes'
BOTTOM_BOXES = '/mongla/sim/bottom_camera/boxes'
FRONT_FX = '/mongla/sim/front_camera/image_fx'
BOTTOM_FX = '/mongla/sim/bottom_camera/image_fx'
FRONT_INFO = '/mongla/sim/front_camera/camera_info'
BOTTOM_INFO = '/mongla/sim/bottom_camera/camera_info'
GROUND_TRUTH = '/mongla/sim/ground_truth'
...
def _course_yaml(course: str) -> Path:
    try:
        share = Path(get_package_share_directory('mongla_sim_worlds'))
        candidate = share / 'courses' / f'{course}.yaml'
        if candidate.is_file():
            return candidate
    except Exception:
        pass
    return _workspace_root() / 'src' / 'mongla_sim_worlds' / 'courses' / f'{course}.yaml'
...
def _fx_params_for(course: str, lighting: str = '') -> dict:
    ...
    try:
        ...
        if lighting and presets.is_file():
            ...
            share = Path(get_package_share_directory('mongla_sim_bridge'))
            params = yaml.safe_load(
