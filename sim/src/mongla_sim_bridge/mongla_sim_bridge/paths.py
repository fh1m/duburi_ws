...
Mongla is **one repo, two colcon workspaces**::
    ...
    mongla_ws/        <- mongla_ws_root()   autonomy: manager, planner, vision
      src/mongla_manager/ ...
      sim/            <- sim_ws_root()      this workspace: Gazebo + SITL + lab
        src/mongla_sim_bridge/ ...

Until 2026-08-26 the sim was a *sibling* tree literally named ``mongla-sim_ws``
and three places guessed at that layout independently:
``record_cameras._workspace_root``, ``mongla_sim_web.server._workspace_root``,
and ``scripts/mongla_sim``. Two of the three matched on the directory *name*,
...
``scripts/mongla_sim`` mirrors the same env → marker-walk → error chain in shell
...
SIM_MARKER = Path('src') / 'mongla_sim_bridge'
AUTONOMY_MARKER = Path('src') / 'mongla_manager'

SIM_WS_ENV = 'MONGLA_SIM_WS'
MONGLA_WS_ENV = 'MONGLA_WS'
...
def sim_ws_root() -> Path:
    ...
    Order: ``$MONGLA_SIM_WS`` → walk up from this file → walk up from the
    ...
    share = _ament_share('mongla_sim_bridge')
    if share:
    ...
    raise RuntimeError(
        ...
        f'Set {SIM_WS_ENV}=<path to mongla_ws/sim>.'
    )


def mongla_ws_root() -> Path:
    """Root of the autonomy workspace (the directory holding ``src/mongla_manager``).

    In-repo this is simply ``sim_ws_root().parent``; the walk keeps a
    sibling checkout working too.
    """
    env = os.environ.get(MONGLA_WS_ENV)
    if env:
        root = Path(env).expanduser().resolve()
        if (root / AUTONOMY_MARKER).is_dir():
            return root
        raise RuntimeError(
            f'{MONGLA_WS_ENV}={env} does not contain {AUTONOMY_MARKER} — '
            f'point it at the mongla_ws checkout root.'
        )
    ...
    raise RuntimeError(
        ...
        f'Set {MONGLA_WS_ENV}=<path to mongla_ws>.'
    )
...
def runtime_dir() -> Path:
    ...
    The lab and the ``mongla_sim`` shell CLI agree on the active course, the lab
    ...
    ``/tmp/mongla_lab_active_course.txt`` and the lab would either fail to write
    it or read someone else's course and restart Gazebo into the wrong world.

    ``/tmp/mongla-$USER/`` at mode 0700 fixes both. ``scripts/mongla_sim``
    ...
    d = Path(tempfile.gettempdir()) / f'mongla-{user}'
