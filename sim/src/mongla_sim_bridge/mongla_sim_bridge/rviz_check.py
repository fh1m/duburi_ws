...
has at least one subscriber while RViz is running.
    ...
    ros2 run mongla_sim_bridge rviz_check                      # installed config
    ros2 run mongla_sim_bridge rviz_check --config path.rviz
...
def main(argv=None):
    ...
    if not path:
        ...
        import os
        path = os.path.join(get_package_share_directory('mongla_sim_bringup'),
                            'config', 'mongla.rviz')
