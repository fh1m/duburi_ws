...
port, so `mongla.fire()` -- and with it `align(fire=..., fire_t=...)`, the
...
single-digit write, same reconnect path. Nothing in `mongla_control` changes,
...
# Must match PAYLOAD_SHOT_SLOTS in mongla_sim_worlds/scripts/gen_world.py --
# a name outside the baked whitelist spawns a projectile with no buoyancy.
SHOT_SLOTS = 12

FIRED_TOPIC = '/mongla/sim/payload/fired'
