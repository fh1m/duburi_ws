...
`RIG FAILED`, never `scorer said miss`.
    ...
    ros2 run mongla_sim_bridge score_check --task torpedo
    ros2 run mongla_sim_bridge score_check --task bins
    ros2 run mongla_sim_bridge score_check              # both
...
mongla_ws". This shells out to the `mongla` CLI exactly as `verb_audit` does.
...
WS = os.environ.get(
    'MONGLA_WS', '/home/fh1m/Envs/dockers/auv-ros2/Ros_workspaces/mongla_ws')
...
def ground_truth():
    ...
    for _ in range(4):
        out = sh('timeout 15 ros2 topic echo /mongla/sim/ground_truth --once '
                 '2>/dev/null')
        try:
...
def place(world, x, y, z, yaw=0.0):
    ...
    sh(f'gz service -s /world/{world}/set_pose --reqtype gz.msgs.Pose '
       ...
       f"'name: \"mongla\", position: {{x: {x}, y: {y}, z: {z}}}, "
...
def cli(verb, args=''):
    return sh(f'timeout 90 ros2 run mongla_planner mongla {verb} {args} 2>&1',
              120)
...
def shots_of(marker):
    ...
    """
    out = sh('timeout 25 ros2 topic echo /mongla/sim/score --once '
