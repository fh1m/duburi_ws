...
It publishes two things deliberately:
  ...
  /mongla/sim/pose_truth     the same pose, as a PoseStamped
  /mongla/sim/pose_believed  where the STACK thinks it is
...
`/mongla/state`'s depth, i.e. exactly the quantities the control loops act on.
...
class SimTf(Node):
    def __init__(self):
        ...
        self.pub_truth = self.create_publisher(
            PoseStamped, '/mongla/sim/pose_truth', 10)
        self.pub_belief = self.create_publisher(
            PoseStamped, '/mongla/sim/pose_believed', 10)
        ...
        if MonglaState is not None:
            ...
            self.get_logger().warn(
                '[TF   ] mongla_interfaces not importable -- publishing truth '
