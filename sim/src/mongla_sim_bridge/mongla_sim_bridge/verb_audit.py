#!/usr/bin/env python3
"""Measure what each /mongla/move verb PHYSICALLY does, against ground truth.
...
So every number here comes from `/mongla/sim/ground_truth`, never from the
verb's own result.
...
runs. Verify yaw sense from the RC PWM instead (Ch4 > 1500 = RIGHT).
    ...
    ros2 run mongla_sim_bridge verb_audit            # all groups
    ros2 run mongla_sim_bridge verb_audit --group yaw
"""
import argparse
import json
import math
...
WS = '/home/fh1m/Envs/dockers/auv-ros2/Ros_workspaces/mongla_ws'
...
def gt():
    ...
    for _ in range(4):
        out = sh(SRC + 'timeout 15 ros2 topic echo /mongla/sim/ground_truth --once 2>/dev/null')
        try:
...
def verb(v, args='', timeout=200):
    out = sh(SRC + f'timeout {timeout} ros2 run mongla_planner mongla {v} {args} 2>&1',
             timeout + 30)
    ok = '-> OK' in out
    msg = ''
...
def group_stateful():
    ...
    for mode in ('ALT_HOLD', 'MANUAL'):
        ...
        time.sleep(2)
        out = sh(SRC + 'timeout 15 ros2 topic echo /mongla/state --once 2>/dev/null')
        got = ''
        for line in out.splitlines():
            if line.startswith('mode:'):
                got = line.split(':', 1)[1].strip()
        record(f'set_mode {mode}', 'OK' if got == mode else 'SUSPECT',
               f'/mongla/state reports {got!r} (ok={ok})')
...
def main():
    ...
    if gt() is None:
        sys.exit('no /mongla/sim/ground_truth -- is the sim running?')
    verb('arm')
