...
"""Assert that the simulator satisfies the drop-in contract mongla_ws expects.
...
rather than by eye:
  ...
  * /mongla/sim/front_camera/image_raw   640x480, rgb8, publishing
  * /mongla/sim/bottom_camera/image_raw  640x480, rgb8, publishing
  * /mongla/sim/{front,bottom}_camera/camera_info  publishing, matching size
  * /mongla/sim/ground_truth             publishing

MAVLink cannot be checked from here; it is not a ROS topic. Run
`ros2 topic echo /mongla/state` against mongla_ws for that half.

Usage:
    ros2 run mongla_sim_bridge contract_check
    ros2 run mongla_sim_bridge contract_check --ros-args -p timeout:=30.0
...
IMAGE_TOPICS = [
    '/mongla/sim/front_camera/image_raw',
    '/mongla/sim/bottom_camera/image_raw',
]
INFO_TOPICS = [
    '/mongla/sim/front_camera/camera_info',
    '/mongla/sim/bottom_camera/camera_info',
]
...
class ContractCheck(Node):
    ...
    def __init__(self):
        super().__init__('mongla_sim_contract_check')
        ...
        # Ground truth is optional in the sense that mongla_ws never reads it,
        ...
        def cb(msg):
            ...
            if msg.width != EXPECTED_WIDTH or msg.height != EXPECTED_HEIGHT:
                self.problems[topic] = (
                    ...
                    'sim_bottom profiles in mongla_vision assume the latter.'
                )
    ...
    def report(self) -> bool:
        ...
        if not self.ground_truth_seen:
            ok = False
        print(f'  {gt:<8} /mongla/sim/ground_truth')

        print('-' * 62)
        if ok:
            print('contract satisfied: mongla_ws can run against this simulator '
                  'unmodified\n')
        else:
            print('contract NOT satisfied - mongla_ws will not see what it '
                  'expects\n')
        return ok
