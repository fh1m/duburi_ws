...
def _course_yaml(world: str) -> dict:
    ...
    try:
        ...
        path = os.path.join(get_package_share_directory('mongla_sim_worlds'),
...
def _prop_library():
    ...
    try:
        ...
        scripts = os.path.join(
            get_package_share_directory('mongla_sim_worlds'), 'scripts')
...
class Scoring(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('vehicle', 'mongla')
        ...
        self.create_subscription(Int32, '/mongla/sim/payload/fired',
                                 self._on_fired, 10)
        ...
        try:
            ...
            self.get_logger().info(
                '[SCORE] mongla_interfaces not on the path -- start the run '
    ...
    def _write_card(self) -> None:
        ...
        you change course -- erased the run you had just done. MONGLA_RUN_DIR
        is the tree the mission scorecards already use, so a practice run's
        two halves land under one timestamp.
        """
        try:
            base = os.environ.get('MONGLA_RUN_DIR') or os.path.expanduser(
                '~/mongla_runs')
