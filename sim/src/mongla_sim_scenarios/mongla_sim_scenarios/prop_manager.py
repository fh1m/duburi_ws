...
Exposes three ROS services under /mongla/sim/props:

    spawn   ros_gz_interfaces/SpawnEntity
    move    ros_gz_interfaces/SetEntityPose
    delete  ros_gz_interfaces/DeleteEntity

Spawn accepts either a registered prop name from mongla_sim_worlds (in
...
Usage:
    ros2 run mongla_sim_scenarios prop_manager --ros-args -p world:=sauvc26_final
"""
...
from mongla_sim_scenarios.prop_catalog import (
    load_prop_library,
    prop_names,
    render_prop,
)
...
class PropManager(Node):
    ...
    def __init__(self):
        super().__init__('mongla_sim_prop_manager')
        ...
        if self.library is None:
            self.get_logger().warning(
                'mongla_sim_worlds prop library not importable; spawn will only '
