...
nothing here imports or links against mongla_ws. Run it in one terminal and the
autonomy stack in another.
    ...
    ros2 launch mongla_manager bringup.launch.py mode:=sim \\
        yaw_source:=mavlink_ahrs flight_controller:=pixhawk \\
        dvl_auto_connect:=false

    # terminal 3
    ros2 run mongla_sim_scenarios gate_transit_check
    ...
    ros2 run mongla_planner mongla lock_heading
    ros2 run mongla_planner mongla move_forward --duration 22 --gain 60
...
def load_gate(course: str, name: str = None):
    ...
    share = get_package_share_directory('mongla_sim_worlds')
...
def main() -> int:
    ...
    rclpy.init()
    node = Node('mongla_sim_gate_transit_check')
    track = []
