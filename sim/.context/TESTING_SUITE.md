...
| Amenity | Why | Status |
|---------|-----|--------|
| PlotJuggler helper + layout | Live state/GT graphs | **shipped** (`mongla_sim plotjuggler`) |
| Foxglove layouts for sim | 3D + images | Use `mongla_ws` foxglove docs |
...
| Hydro / thruster step panel | Vehicle ID from scripts | scripts in `mongla_sim_worlds` |
| Turbidity schedule presets | Domain randomization | FX API ready; presets planned |
| Multi-clip batch + train handoff | Dataset → YOLO in `mongla_ws` | docs only |
