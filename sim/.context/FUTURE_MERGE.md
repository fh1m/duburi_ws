...
**Settled 2026-08-27.** The simulator was absorbed into the `mongla_ws` repo as
the nested colcon workspace **`mongla_ws/sim/`**. It is under version control and
also published standalone as
[`fh1m/mongla-sim_ws`](https://github.com/fh1m/mongla-sim_ws).
**`mongla_ws/sim/` is canonical; the standalone repo mirrors it.**
...
| Packages under `mongla_ws/src/mongla_sim_*` | ✗ | root `colcon build` / `colcon test` / `pytest src/` would pick up six Gazebo-dependent packages; autonomy must stay *identically* robust, i.e. the same test set, not a superset |
...
- Two copies exist (`mongla_ws/sim/` and the mirror repo). Drift is possible; the
...
- Avoids forcing Gazebo/ArduPilot deps onto every `mongla_ws` checkout
...
1. `git init` + push as Option B, then submodule into `mongla_ws`, or  
2. Copy/subtree into `mongla_ws` first (Option A) then commit there.

See [HANDOFF.md](HANDOFF.md).
## Option A — subtree inside `mongla_ws` (recommended later)

Target sketch:

```text
mongla_ws/
  sim/                    # or workspaces/mongla_sim/
    src/mongla_sim_*
    .context/
  src/mongla_manager/     # existing
  ...
```
...
2. Preserve package names (`mongla_sim_*`) to avoid ament index churn.  
3. Update `MONGLA_WS` discovery: lab/server currently walks parents for `mongla_ws/sim` / sibling.  
...
6. Docs: move `.context/` with the tree; add pointer from `mongla_ws/CLAUDE.md`.  
7. CI: job matrix — build sim packages only when `sim/**` changes.  
8. Deprecate sibling README with redirect for one release.

## Option B — dedicated public `mongla-sim` GitHub repo

- Extract current workspace as its own remote.  
- Document required sibling/or submodule pin of `mongla_ws` for stack.  
...
## Option C — git submodule under `mongla_ws/sim`

- `mongla_ws` pins sim SHA.  
