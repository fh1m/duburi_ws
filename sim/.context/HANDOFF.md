# Handoff — Cursor agent → next Claude / mongla_ws agent
...
**Origin:** built 2026-08-26; absorbed into the `mongla_ws` repo 2026-08-27.  
**Audience:** next Claude Code / Cursor agent continuing sim work, and the
`mongla_ws` agent integrating autonomy against this sibling.
...
5. Drop-in MAVLink/camera **contract** so `mongla_ws` missions run unmodified  

Autonomy code stays in `../mongla_ws`. **Do not merge packages** until the user
asks — recipe only in [FUTURE_MERGE.md](FUTURE_MERGE.md).

## Git status (important)

`mongla_ws/sim` is **under git**, inside the `mongla_ws` repo (2026-08-27). It previously had none; see [FUTURE_MERGE.md](FUTURE_MERGE.md) for the decision.

- The `mongla_ws` agent will initialize / submodule / subtree when integrating.  
- Land changes **here**; `fh1m/mongla-sim_ws` is a published mirror, not the source.  
...
https://github.com/fh1m/mongla_ws/pull/8 — branch `docs/mongla-sim-sibling-pointer`.
...
| Layer | Owner | Why |
|-------|-------|-----|
| Physics + SITL + cams | `mongla_ws/sim` | Gazebo/ArduPilot deps stay off every autonomy checkout |
| Manager / planner / vision / missions | `mongla_ws` | Same binaries as the pool vehicle |
| Operator lab | `mongla_sim_web` | FastAPI+React local tool, not embedded PlotJuggler/Qt |
...
4. **PlotJuggler** — `mongla_sim plotjuggler` + layout XML + docs.  
5. **Stop orphans** — `mongla_sim stop` also kills lab / recorder / bridges / prop_manager.  
...
mongla_ws/sim/
  ...
  src/
    mongla_sim_description/
    mongla_sim_worlds/
    mongla_sim_bringup/   # mongla_sim CLI, sim/stack launch
    mongla_sim_bridge/    # cams, FX, record_cameras, checks
    mongla_sim_scenarios/ # props
    mongla_sim_web/       # lab API + frontend/ → static/
...
6. Done 2026-08-27: absorbed into `mongla_ws` as `sim/` (see FUTURE_MERGE).

For **`mongla_ws` Claude agent**:

1. Merge/review [PR #8](https://github.com/fh1m/mongla_ws/pull/8).  
...
```zsh
# read
less ~/Ros_workspaces/mongla_ws/sim/.context/INDEX.md
less ~/Ros_workspaces/mongla_ws/sim/.context/AUDIT.md
...
# after UI edits
cd src/mongla_sim_web/frontend && npm run build
colcon build --packages-select mongla_sim_web --symlink-install
```

Hard rules: one sim; prefer this `.context/` over `mongla_ws` `sim-setup.md`;
...
[TESTING.md](TESTING.md). When talking to the `mongla_ws` agent, point them at
