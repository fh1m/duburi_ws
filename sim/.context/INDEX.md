# `mongla_ws/sim` — agent / operator index
...
Nested inside [`mongla_ws`](../..); autonomy lives in [`../../src`](../../src). MAVLink/camera surface so autonomy
...
| Fact | Detail |
|------|--------|
| Workspace root | `Ros_workspaces/mongla_ws/sim` |
| Autonomy sibling | `Ros_workspaces/mongla_ws` (`MONGLA_WS`) |
| Helper CLI | `ros2 run mongla_sim_bringup mongla_sim <cmd>` |
...
| Operator UI | HTTP **28765** (`MONGLA_LAB_PORT`, auto-fallback) |
| Cameras | `/mongla/sim/{front,bottom}_camera/image_raw` 640×480 |
| One-sim rule | Always `mongla_sim stop` before a new `sim` |

Canonical bring-up:

```zsh
mongla_sim stop → mongla_sim sim → mongla_sim stack --no-vision → mongla_sim smoke
# optional: mongla_sim lab  →  http://localhost:28765
```
...
| [HANDOFF.md](HANDOFF.md) | Next agent / mongla_ws agent | What Cursor built, why, packaging, next work |
...
| [INTEGRATION_DUBURI_WS.md](INTEGRATION_DUBURI_WS.md) | `mongla_ws` devs | `srot` vs `main`, pixhawk, vision remap |
...
| [FUTURE_MERGE.md](FUTURE_MERGE.md) | Maintainers | Sibling → `mongla_ws` / separate repo |
...
2. On `mongla_ws` **`srot`**, stack must use `flight_controller:=pixhawk` (already set by `stack.launch.py`).
3. Prefer this `.context/` over `mongla_ws/.claude/context/sim-setup.md` (legacy BlueROV/`colcon_ws`).
...
7. This tree is **under git** inside `mongla_ws` (2026-08-27). Land changes here; `fh1m/mongla-sim_ws` mirrors it. Stage by path — never `git add -A`
...
| Package | Role |
|---------|------|
| `mongla_sim_description` | Vehicle SDF / hydro configs |
| `mongla_sim_worlds` | Pool, props, course YAML → `.world` |
| `mongla_sim_bringup` | `sim` / `stack` / `mongla_sim` helper |
| `mongla_sim_bridge` | ros_gz cams/GT, FX, recorder, checks |
| `mongla_sim_scenarios` | Runtime prop spawn/move/delete |
