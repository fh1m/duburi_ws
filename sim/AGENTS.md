# AGENTS.md — agent entry for `mongla_ws/sim`

## Purpose

Gazebo Harmonic + ArduSub SITL simulator and operator lab for Mongla / Duburi,
a nested colcon workspace inside the `mongla_ws` repo (autonomy lives in `../src`).
...
- Follow one-sim rule; use `mongla_sim` helper.
...
- Rebuild `src/mongla_sim_web/frontend` → `static/` after UI edits.
...
- Treat `mongla_ws/.claude/context/sim-setup.md` as current.
...
`contract_check`, `mavlink_check`, `mongla_sim smoke`, lab `/api/health` —
