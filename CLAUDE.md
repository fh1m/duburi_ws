...
| `pixhawk.gain_to_pwm(value, pass_through)` | `pass_through=False` (default) = percent→PWM (`percent_to_pwm`); `True` = **raw PWM delta** `1500+value` (clamped 1100-1900). The `pass_through=True` flag on `move_forward/back/left/right/arc` reinterprets `gain` as raw PWM so you can probe the minimum PWM that moves the 20 kg hull (`move_forward --gain 4 --pass_through` → Ch5=1504; no reverse-kick brake in this mode). `yaw_left/right` are PID/angle-targeted — no gain knob, so no `pass_through`. |
...
| `vision.anchor_snap(name=, target=, conf=, err=)` / `vision.anchor_align(name, err=, theta=, hold=, fire=, match=, gain=, …)` / `vision.anchor_clear()` | `vision_anchor_snap`/`_align`/`_clear` | **★ branch `lock`.** XFeat+LighterGlue geometric "superglue": snap a reference frame, then drive lat/yaw/depth to re-superimpose the live view on it (homography pose error) — a fine lock that **holds with no YOLO bbox** (torpedo hole up close). `hold=`s active station-keep; `fire=[1,2]` fires once mid-hold at lock. `anchor_snap('hole')` saves `references/hole.png`; `anchor_align('hole')` reloads it (reproducible runs). **`anchor_snap(target='hole', conf=, err=)`** = snap-at-detection: waits ≤3 s for that detection near centre and snaps **just its bbox crop** (whole-frame fallback after 3 s); cropped keypoints are offset to full-frame coords so signs are unchanged. **Homography is ref→live so signs match `align_loop`** (swapping inverts to runaway); `theta`→yaw is image-roll on the forward cam (deadbanded — weak axis there). No forward axis (monocular = no range). `anchor:=false` default; needs `torch.hub` weights pre-downloaded on the Jetson (warm LighterGlue with one real `match()`). **Redesigned command surface (branch `lock`): prefer the `duburi.anchor.*` namespace** — `anchor.snap(source='detection'\|'frame', target=, conf=, err=)` / `anchor.save(name, …)` / `anchor.align(ref=, hold=, fire=, match=)` / `anchor.clear()` (the `vision.anchor_*` names above stay as back-compat aliases). **`vision.align(use_feature=True)`** fuses the anchor pose as a *detection fallback* inside the normal align loop (detector primary; substitutes the homography pose only when YOLO drops a tick AND the anchor is LOCKED — cloudy-day robustness that outlasts the tracker coast). Usage/patterns: [`anchor-patterns.md`](.claude/context/anchor-patterns.md) · Jetson deploy: [`xfeat-setup.md`](.claude/context/xfeat-setup.md) · architecture: [`anchor-system.md`](.claude/context/anchor-system.md). Litmus mission: `ros2 run duburi_planner mission xfeat_torpedo`. |
...
| `anchor-system.md`              | **★ branch `lock`** — XFeat+LighterGlue superglue lock: verbs, axis mapping (tx→lat/theta→yaw/ty→depth, no range), Ch4 arbitration, pool-day torch.hub prereq, phase-2 backlog |
| `anchor-patterns.md`            | **★ branch `lock`** — how to USE the XFeat lock: 4 patterns (torpedo stick-like-glue, cloudy-day standalone fallback, precise find→hold, `use_feature` fused fallback), `duburi.anchor.*` namespace, litmus mission `xfeat_torpedo`, texture/no-forward caveats |
| `xfeat-setup.md`                | **★ branch `lock`** — on-Jetson deploy runbook: XFeat+LighterGlue weights pre-download (canary), numpy/torch pins, VRAM budget vs YOLO-TRT, `anchor:=true` launch+params, optional ONNX/TensorRT, troubleshooting |
