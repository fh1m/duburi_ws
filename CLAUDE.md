...
| `vision.anchor_snap()` / `vision.anchor_align(err=, theta=, hold=, fire=, match=, gain=, …)` / `vision.anchor_clear()` | `vision_anchor_snap`/`_align`/`_clear` | **★ branch `lock` (phase 1).** XFeat+LighterGlue geometric "superglue": snap a reference frame, then drive lat/yaw/depth to re-superimpose the live view on it (homography pose error) — a fine lock that **holds with no YOLO bbox** (torpedo hole up close). `hold=`s active station-keep; `fire=[1,2]` fires once mid-hold at lock. No forward axis (monocular = no range). `anchor:=false` by default. Full ref: [`anchor-system.md`](.claude/context/anchor-system.md). |
