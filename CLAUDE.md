...
- **Precision terminal alignment (close-in robustness, all opt-in/off by default):** `align(lock_on=True)` (continuity lock — kills last-moment misclassification), and three deck ROS params — `vision.range_gain_floor` (softens lat/depth gain as the bbox fills → stops the 20 kg hull overshooting up close; `1.0`=off, `~0.3`=gentle), `vision.ctrl_conf` (control-side conf floor, distinct from the detector's `conf`), `vision.ki_lat` (lateral-only integral, nulls a steady current during the hold; enable **after** damping). At the hole, **drop the `yaw` axis** and let `heading_lock` hold Ch4 (no vision-yaw wobble). Full guide + pool runbook + "what NOT to do": [`precision-alignment.md`](.claude/context/precision-alignment.md).
