#!/usr/bin/env bash
# Build Mongla_agile/Mongla packages and fix two Debian-vs-ament install quirks:
...
#
# Build order matters: mongla_interfaces first (ament_cmake, generates Move.action),
# then the Python packages (control, sensors, manager, planner) which depend on
# the generated types and on mongla_control's COMMANDS registry.
#
...
#     ~/models    -> src/mongla_vision/models/   (put the Pi's .hef here too --
#                    the sync copies every regular file, so nothing to change)
#     ~/missions  -> src/mongla_planner/mongla_planner/missions/
# Every build mirrors those folders INTO the tree first, so a fresh `git clone`
# gets them back on the next `./build_mongla.sh` with nothing to copy by hand.
# Override the source dirs with MONGLA_MODELS_SRC / MONGLA_MISSIONS_SRC.
# On a dev box without those folders the sync is skipped (build still works).
set -e
cd "$(dirname "$0")"

MODELS_SRC="${MONGLA_MODELS_SRC:-$HOME/models}"
MISSIONS_SRC="${MONGLA_MISSIONS_SRC:-$HOME/missions}"
MODELS_DST="src/mongla_vision/models"
MISSIONS_DST="src/mongla_planner/mongla_planner/missions"
...
# mongla_interfaces is ament_cmake so its install dir is managed correctly by CMake.
for pkg in mongla_control mongla_manager mongla_sensors mongla_localization mongla_planner mongla_vision; do
...
colcon build --packages-select mongla_interfaces --cmake-args -Wno-dev "$@"
...
echo "Then:"
echo "  ros2 run mongla_manager auv_manager --ros-args -p mode:=sim"
echo "  ros2 run mongla_planner mongla <cmd> [--field value ...]"
echo "  ros2 run mongla_planner mission demo_square"
echo "Vision (single camera + YOLO11 detector + tracker + viewer):"
echo "  ros2 launch mongla_vision vision.launch.py camera:=forward"
echo "Vision (dual camera, competition):"
echo "  ros2 launch mongla_vision vision_dual.launch.py"
