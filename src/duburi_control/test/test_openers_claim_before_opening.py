...
for _p in ('duburi_control', 'duburi_sensors'):
...
def test_the_bno_claims_the_port_before_opening_it():
    p = os.path.join(_WS, 'src', 'duburi_sensors', 'duburi_sensors',
                     'sources', 'bno085.py')
...
def test_the_payload_claims_the_port_before_opening_it():
    p = os.path.join(_WS, 'src', 'duburi_control', 'duburi_control', 'payload.py')
    order = _order_of(p, [('acquire', 'self._guard.acquire()'),
                          ('open',    '_p.open()')])
    assert order[:2] == ['acquire', 'open'], (
...
def test_both_release_the_claim_when_they_close():
    ...
    bno = open(os.path.join(_WS, 'src', 'duburi_sensors', 'duburi_sensors',
                            'sources', 'bno085.py')).read()
    pay = open(os.path.join(_WS, 'src', 'duburi_control', 'duburi_control',
                            'payload.py')).read()
...
def test_the_guard_is_inert_for_the_sim_pty():
    ...
    """
    from duburi_control.fc.port_guard import PortGuard, is_serial
    pty = f'/tmp/duburi-{os.environ.get("USER", "x")}/bno085'
