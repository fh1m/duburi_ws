...
So the failure mode is not theoretical. `duburi_ws` has FOUR places that open
...
def _lock_dir() -> Path:
    ...
    """
    base = os.environ.get('XDG_RUNTIME_DIR') or f'/tmp/duburi-{os.environ.get("USER", os.getuid())}'
...
class PortBusy(RuntimeError):
    """Raised instead of opening a port another duburi process already owns."""
