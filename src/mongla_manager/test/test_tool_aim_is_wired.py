...
def test_the_resolver_lives_where_the_dependency_allows():
    """`mongla_control` must not import `mongla_vision`. The geometry table is
    in vision, so the manager's VisionState resolves it -- the same rule that
    keeps the refractive index out of bearing.py."""
    assert 'from mongla_vision.tool_geometry import pixel_offset' in _VS
    assert 'mongla_vision' not in _MV, 'motion_vision imports mongla_vision'
...
_sys.path.insert(0, str(Path(__file__).resolve().parents[2] / 'mongla_control'))
from mongla_control.vision_verbs import VisionVerbs      # noqa: E402
