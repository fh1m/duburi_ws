...
def test_uplink_really_does_send_coasted_and_gap_age():
    ...
    """
    import ast
    node = (_WS / 'src' / 'mongla_manager' / 'mongla_manager' / 'auv_manager_node.py')
...
def test_the_motion_guard_blocker_is_recorded_as_gone():
    ...
    diff = subprocess.run(
        ['git', '-C', str(_WS), 'diff', '--stat', 'main', 'srot', '--',
         'src/mongla_control/mongla_control/motion_forward.py',
         'src/mongla_control/mongla_control/motion_lateral.py'],
