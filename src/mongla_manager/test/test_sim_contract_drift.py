...
Mongla is one repo with two colcon workspaces: ``mongla_ws/`` (this one, the same
binaries that fly on the vehicle) and ``mongla_ws/sim/`` (Gazebo Harmonic +
...
``mongla_control/test/test_srot_protocol_drift.py``: read the OTHER side's source
...
from mongla_manager.connection_config import PROFILES

REPO = Path(__file__).resolve().parents[3]
SIM = REPO / 'sim'

pytestmark = pytest.mark.skipif(
    not (SIM / 'src' / 'mongla_sim_bringup').is_dir(),
    reason='sim/ not present in this checkout (sparse clone) -- nothing to referee',
)

STACK_LAUNCH = SIM / 'src/mongla_sim_bringup/launch/stack.launch.py'
SIM_LAUNCH = SIM / 'src/mongla_sim_bringup/launch/sim.launch.py'
CONTRACT_CHECK = SIM / 'src/mongla_sim_bridge/mongla_sim_bridge/contract_check.py'
BRIDGE_LAUNCH = SIM / 'src/mongla_sim_bridge/launch/bridge.launch.py'
TELEOP = SIM / 'src/mongla_sim_web/mongla_sim_web/teleop.py'
SUB_PARM = SIM / 'src/mongla_sim_bringup/config/mongla_sub.parm'
VEHICLE_SDF = SIM / 'src/mongla_sim_description/models/mongla_heavy/model.sdf'

MANAGER_LAUNCH = REPO / 'src/mongla_manager/launch/bringup.launch.py'
VISION_LAUNCH = REPO / 'src/mongla_vision/launch/vision.launch.py'
VISION_DUAL_LAUNCH = REPO / 'src/mongla_vision/launch/vision_dual.launch.py'
...
def test_every_manager_arg_the_sim_passes_is_real():
    ...
    assert not unknown, (
        f'stack.launch.py passes {sorted(unknown)} to mongla_manager/bringup.launch.py, '
...
def test_every_vision_arg_the_sim_passes_is_real():
    ...
    assert not unknown, (
        ...
        f'mongla_vision/vision_dual.launch.py, which does not declare them -- '
...
def test_sim_vision_drives_both_cameras_from_the_sim_topics():
    ...
    (`/mongla_detector_forward`, `_vision_state_for('downward')`), and the sim
...
def test_lab_teleop_does_not_steal_the_managers_socket():
    ...
    """
    # DEFAULT_ENDPOINT = os.environ.get('MONGLA_TELEOP_ENDPOINT', 'tcp:...')
...
def test_camera_topics_agree_across_sim_bridge_stack_and_check():
    src = STACK_LAUNCH.read_text()
    topic = re.search(r"default_value='(/mongla/sim/\S+?)'", src).group(1)
...
def test_thruster_pwm_range_agrees_between_the_model_and_ardusub():
    ...
    assert f'<servo_min>{lo}</servo_min>' in sdf, (
        f'mongla_sub.parm says MOT_PWM_MIN={lo}; the Gazebo plugin disagrees. '
    ...
    assert f'<servo_max>{hi}</servo_max>' in sdf, (
        f'mongla_sub.parm says MOT_PWM_MAX={hi}; the Gazebo plugin disagrees.'
    )
...
def test_every_course_has_a_turbidity_sidecar_beside_its_world():
    ...
    """
    worlds = sorted((SIM / 'src/mongla_sim_worlds/worlds').glob('*.world'))
...
def test_the_sidecar_carries_the_params_underwater_fx_declares():
    ...
    fx_node = (SIM / 'src/mongla_sim_bridge/mongla_sim_bridge/'
    ...
    sidecar = SIM / 'src/mongla_sim_worlds/worlds/sauvc26_final.fx.yaml'
...
def test_the_murky_preset_is_actually_murkier_than_the_clear_one():
    ...
    def turbidity(course):
        path = SIM / f'src/mongla_sim_worlds/worlds/{course}.fx.yaml'
...
def test_dvl_beam_visuals_are_off_in_the_generated_model():
    ...
    """
    sdf = (SIM / 'src/mongla_sim_description/models/mongla_heavy/'
                 'model.sdf').read_text()
...
def _prop_library():
    ...
    path = SIM / 'src/mongla_sim_worlds/scripts/prop_library.py'
...
def test_a_world_is_generated_against_its_own_competition():
    ...
    """
    import re
    worlds = SIM / 'src/mongla_sim_worlds/worlds'
    courses = SIM / 'src/mongla_sim_worlds/courses'
...
def test_lab_pool_profile_matches_the_arena_specs():
    ...
    pl = _prop_library()
    js = (SIM / 'src/mongla_sim_web/frontend/src/App.jsx').read_text()
...
def test_floor_anchored_props_sit_on_the_sloped_floor():
    ...
    world = (SIM / 'src/mongla_sim_worlds/worlds/sauvc26_final.world').read_text()
...
def test_courses_stay_within_a_render_budget():
    ...
    worlds = SIM / 'src/mongla_sim_worlds/worlds'
    models = SIM / 'src/mongla_sim_worlds/models'
...
def test_thrust_test_rigs_document_the_t200_conflict():
    ...
    """
    results = (SIM / 'src/mongla_sim_description/models/mongla_heavy/'
                     'RESULTS.md')
