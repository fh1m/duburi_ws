...
_CHECK = os.path.join(_WS, 'src', 'mongla_manager',
                      'mongla_manager', 'bringup_check.py')
...
def test_the_shared_constant_is_the_one_the_manager_declares():
    ...
    from mongla_manager.connection_config import DEFAULT_FLIGHT_CONTROLLER
    node = open(os.path.join(_WS, 'src', 'mongla_manager', 'mongla_manager',
...
def test_the_exclude_survives_a_by_id_symlink():
    ...
    """
    from mongla_control.payload import PayloadDriver
    src = open(os.path.join(_WS, 'src', 'mongla_control', 'mongla_control',
                            'payload.py')).read()
