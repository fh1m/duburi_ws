...
One-command pool-day bringup (defaults: pool mode, DVL auto-connect):
    ...
    ros2 launch mongla_manager bringup.launch.py vision:=true flow:=true \\
        pool_depth_m:=1.2
...
Multi-model registry (switch models mid-mission without restart):
    ...
    # In the mission DSL:
    #   mongla.use('gate')               # phase: gate
    #   mongla.use('flare', 'flare')     # phase: flare
    #   mongla.use('combined', 'gate')   # phase: combined model, gate filter

DVL connects automatically on startup (dvl_auto_connect:=true by default).
Manual override: ros2 run mongla_planner mongla dvl_connect

Vision commands to test gate detection:
    ros2 run mongla_planner mongla vision_align --camera forward --target_class gate --axes yaw,lat --duration 10
    ros2 run mongla_planner mongla vision_move  --camera forward --target_class gate --fwd_fill 80 --mode area --duration 15
...
def generate_launch_description():
    args = [
        ...
        # had to drop to `ros2 run mongla_manager start --ros-args -p ...`, which
        ...
        DeclareLaunchArgument('localization', default_value='true',
                              description='Run the invariant filter (mongla_'
                                          'localization). Read-only: it '
                                          'publishes /mongla/odom and commands '
        ...
        # switched on.
        #
        # The role is NOT renamed: /mongla/vision/forward/... and
        # mongla_detector_forward are what 96 call sites and the DSL's own
        ...
        DeclareLaunchArgument('camera_profile', default_value='pi_forward',
                              description='Camera HARDWARE profile (CAMERA_PROFILES in '
                                          'mongla_vision/config.py -- the loaded copy). '
    ...
    manager_node = Node(
        ...
        ros_arguments=['--log-level', 'warn', '--log-level', 'mongla_manager:=info'],
        parameters=[{
    ...
    vision_launch_path = os.path.join(
        get_package_share_directory('mongla_vision'),
    ...
    # (/mongla/imu, /mongla/state) and it must run with vision:=false too.
    localization_node = Node(
        package='mongla_localization',
        executable='localization_node',
        name='mongla_localization',
        output='screen',
        ros_arguments=['--log-level', 'warn',
                       '--log-level', 'mongla_localization:=info'],
        parameters=[{
    ...
    foxglove_node = Node(
        ...
        parameters=[{
            ...
            # /mongla/move/_action/feedback + /status live under the _action
