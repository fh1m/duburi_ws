...
RIEKF's body velocity and attitude on `/mongla/odom`.
...
WHY VELOCITY, NOT A DIFFERENCE OF POSITIONS. `/mongla/odom` also carries a
...
class FlowPositionSource:
    ...
    def __init__(self, node, inner, camera: str = 'downward',
        ...
        self._sub_odom = node.create_subscription(
            Odometry, '/mongla/odom', self._on_odom, 10)
        self._log.info(
            f'[FLOWP] leg displacement from /mongla/odom velocity, gated on '
    ...
    def reset_position(self) -> None:
        ...
        if not ok:
            ...
            raise MovementError(
                ...
                f'(ros2 topic echo /mongla/vision/downward/flow_quality) and '
    ...
    def position_ready(self) -> Tuple[bool, str]:
        ...
        if odom_t is None:
            return False, ('no /mongla/odom has arrived (is the localization '
                           'node running?)')
        odom_age = now - odom_t
        if odom_age > self._fix_stale_s:
            return False, f'the last /mongla/odom is {odom_age:.1f} s old'
