...
instead of mid-mission.  Sections:
  ...
  A. Compute / environment   ROS sourced, mongla pkgs import, disk free
...
Usage:
    ...
    ros2 run mongla_manager bringup_check --strict
    ros2 run mongla_manager bringup_check --srot        # SROT-based vehicle
"""
...
def _check_ros_env() -> list[tuple[str, str, str]]:
    ...
    for pkg in ('mongla_control', 'mongla_interfaces', 'mongla_planner'):
        try:
            __import__(pkg)
...
# --------------------------------------------------------------------------- #
# B. vision dependencies (the JetPack pitfalls, pi-and-env-traps.md E1-E3)
# --------------------------------------------------------------------------- #
def _check_vision_deps() -> list[tuple[str, str, str]]:
    out: list[tuple[str, str, str]] = []

    # numpy<2 -- numpy 2 breaks the cv_bridge / cv2 ABI => every vision node
    # dies with `_ARRAY_API not found` (pi-and-env-traps.md E1).
    try:
        import numpy
    ...
    # it headless and kills vision_display (pi-and-env-traps.md E2).
    try:
        import cv2
    ...
    try:
        ...
        if shadows:
            out.append((WARN, 'pip opencv installed',
                        ...
                        'pip uninstall them (pi-and-env-traps.md E2)'))
    except Exception:
        pass
...
# --------------------------------------------------------------------------- #
# C. serial drivers (payload CH340 + brltty trap, pi-and-env-traps.md E5)
# --------------------------------------------------------------------------- #
def _check_serial_drivers() -> list[tuple[str, str, str]]:
    ...
    if have_ch341 is None:
        ...
        out.append((WARN, 'ch341 driver MISSING',
                    ...
                    'tools/install_ch341_driver.sh (pi-and-env-traps.md E5)'))
    ...
    try:
        ...
        if '\nii  brltty' in installed or installed.startswith('ii  brltty'):
            out.append((WARN, 'brltty installed',
                        ...
                        'sudo apt-get purge -y brltty (pi-and-env-traps.md E5)'))
        else:
...
def _check_bno085_auto() -> tuple[str, str]:
    try:
        from mongla_sensors.sources.bno085 import auto_detect_port
    except Exception as exc:  # noqa: BLE001
        return WARN, f'mongla_sensors not importable yet: {exc}'
    try:
...
def _check_payload() -> tuple[str, str]:
    ...
    """
    try:
        from mongla_control.payload import PayloadDriver
    except Exception as exc:  # noqa: BLE001
        return WARN, f'mongla_control.payload not importable: {exc}'
...
def _models_dirs() -> list[str]:
    ...
    cands: list[str] = []
    # Walk up from this file (installed under .../mongla_ws/install/...) and
    # from the cwd looking for a workspace root that has src/mongla_vision/models.
    for start in (here, os.getcwd()):
        d = start
        for _ in range(8):
            probe = os.path.join(d, 'src', 'mongla_vision', 'models')
    ...
    try:
        ...
        cands.append(os.path.join(
            get_package_share_directory('mongla_vision'), 'models'))
    except Exception:
        pass
    seen: list[str] = []
    for c in cands:
...
def _read_param(conn, name: str, tries: int = 3, timeout_s: float = 1.2):
    ...
    """
    import mongla_control.fc.srot_protocol as sp      # noqa: F811
...
def _depth_loop_verdict(depth_cmd: float | None,
    ...
    if abs(depth_cmd) >= sp.DEPTH_CMD_SATURATED:
        return (FAIL, 'barometer IMPLAUSIBLE',
                ...
                'depth it cannot express. ' + 'Arming would command FULL vertical thrust (mixer throttle column is -1 on all four verticals) with the horizontals idle. DO NOT ARM. If the barometer VARIANCE line above is PASS this is a large zero offset, not a dead sensor -- run `ros2 run mongla_planner mongla calibrate_depth` and re-check')
    ...
    # `depth telemetry` line printed a few rows above, MonglaState, set_depth -- is
    ...
    if abs(depth_m) > sp.DEPTH_OFFSET_WARN_M:
        return (WARN, 'barometer offset at the surface',
                ...
                '`ros2 run mongla_planner mongla calibrate_depth` before diving')
...
# sessions on the Pi (~/mongla_runs/bench_20260902_224032.tlog, txrx_...224254).
...
def _check_srot(skip_mav: bool, device: str = '') -> list[tuple[str, str, str]]:
    ...
    try:
        from pymavlink import mavutil
        # NOT `from mongla_control.fc import srot_protocol`: that runs
        # mongla_control/__init__.py, which imports Mongla -> mongla_interfaces, so it
        ...
        # mongla_interfaces properly -- having the board section die of the same cause
        ...
        if sp is None:
            out.append((FAIL, 'srot_protocol unavailable',
                        'cannot decode modes/ACKs -- rebuild: ./build_mongla.sh'))
            return out
    except Exception as exc:                       # noqa: BLE001
...
def main(argv: list[str] | None = None) -> int:
    ...
    # `ros2 run mongla_manager bringup_check` that CLAUDE.md tells the operator to
    ...
    print('=' * 72)
    print(' mongla bringup_check -- RoboSub pre-flight'
...
def _print_launch_hint(srot: bool = False) -> None:
    if srot:
        ...
        print('    ros2 launch mongla_manager bringup.launch.py   '
              '# srot + mavlink_ahrs are the defaults')
        print('      # fire(N) = BOARD channel N -- see the [PAYLOAD] roles line;'
              ' nothing else needs setting')
        print('    ros2 run mongla_planner mongla arm')
        print('    ros2 run mongla_planner mongla move_forward --duration 5 --gain 40')
        print('    ros2 run mongla_planner mongla stop')
        print()
        ...
        print('    ros2 run mongla_planner mongla set_mode --target_name DEPTH_HOLD')
        ...
        return
    print('    ros2 run mongla_manager start              '
          '# mode=auto picks the right profile')
    print('    ros2 launch mongla_vision vision.launch.py camera:=forward  '
          '# vision pipeline (optional)')
    print('    ros2 run mongla_planner mission move_and_see   '
          '# demo mission')
    print()
    print('  DVL distance commands (pool, yaw_source=dvl or bno085_dvl):')
    print('    ros2 run mongla_planner mongla move_forward_dist --distance_m 2.0')
