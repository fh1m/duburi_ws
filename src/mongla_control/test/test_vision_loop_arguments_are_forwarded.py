...
_PKG = pathlib.Path(__file__).resolve().parents[1] / 'mongla_control'
...
def _advertised():
    ...
    import sys
    sys.path.insert(0, str(_PKG.parents[1] / 'mongla_manager'))
    from mongla_manager.vision_tunables import VISION_PARAM_DEFAULTS
    deck = {k.split('.', 1)[1] for k in VISION_PARAM_DEFAULTS if k.startswith('vision.')}
    action = _PKG.parents[1] / 'mongla_interfaces' / 'action' / 'Move.action'
    fields = set()
    if action.exists():
