...
The register filed that as "latent, not live", reasoning that `Mongla.lock`
...
the conclusion was wrong, because the collision is not scoped-vs-scoped:
  ...
  1. `lock_heading` while DISARMED latches a suspend that is DELIBERATELY
     unpaired. mongla.py says why: "Suspend BEFORE start so the daemon never
...
from mongla_control.heading_lock import HeadingLock          # noqa: E402
...
def _scoped(lk):
    """`mongla._suspend_heading_lock`, transcribed."""
    lk.suspend()
    try:
        yield
    finally:
...
def test_it_matches_the_sibling_primitive_it_should_always_have_matched():
    ...
    from mongla_control.heartbeat import Heartbeat
    import inspect
