...
from mongla_control.fc.base import FIRE_THRUSTER_FAULT, FIRE_NOT_READY
...
def _mongla(verdict, payload=None):
    d = Mongla.__new__(Mongla)                 # no MAVLink, no threads
...
def test_a_KNOWN_BAD_thruster_REFUSES_the_shot():
    ...
    pl = _Payload()
    res = _mongla((False, 'thruster 3 LOST telemetry mid-session'), pl)._fire_payload(1)
...
def test_UNKNOWN_thruster_health_still_FIRES():
    ...
    """
    pl = _Payload()
    res = _mongla((None, 'presence announced at first arm only'), pl)._fire_payload(2)
...
def test_a_HEALTHY_hull_FIRES():
    pl = _Payload()
    assert _mongla((True, 'all 8 thrusters reporting'), pl)._fire_payload(3).ok
    assert pl.fired == [3]
...
def test_a_RAISING_probe_is_not_a_hull_fault():
    ...
    pl = _Payload()
    assert _mongla(RuntimeError('link down'), pl)._fire_payload(1).ok
...
def test_the_gate_runs_BEFORE_the_payload_readiness_check():
    ...
    d = _mongla((False, 'thruster 7 reports no telemetry'), payload=None)
