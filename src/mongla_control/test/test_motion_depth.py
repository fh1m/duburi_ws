...
from mongla_control.motion_depth import (
    _fresh_depth, wait_for_depth, _ATTITUDE_STALE_S, TOL_M,
)
from mongla_control.errors import MovementTimeout
...
def test_wait_for_depth_reaches_when_fresh_at_target(monkeypatch):
    monkeypatch.setattr('mongla_control.motion_depth.time.sleep', lambda *_: None)
...
def test_wait_for_depth_does_not_false_reach_on_stale(monkeypatch):
    ...
    t = {'now': 0.0}
    monkeypatch.setattr('mongla_control.motion_depth.time.monotonic',
...
def test_make_writers_depth_keepalive_releases_ch3_both_lockstates():
    ...
    from mongla_control.pixhawk import NO_OVERRIDE, CH_THROTTLE
