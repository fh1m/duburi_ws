...
def _make(armed=True, ex=0.6):
    ...
    mongla = Mongla(pix, log, heartbeat=hb, vision_state_provider=lambda _cam: vis)
    return mongla, pix, hb
...
def test_yaw_align_suspends_lock_and_retargets_to_final_heading():
    ...
    mongla, pix, hb = _make(armed=True)
    mongla.lock_heading(target=90.0, timeout=30.0)
    ...
    mongla.vision_align(camera='forward', target_class='gate', axes='yaw',
    ...
    assert abs(mongla._heading_lock.target_deg - 10.0) < 0.5, (
...
def test_lat_only_align_keeps_lock_and_releases_ch4():
    ...
    mongla.vision_align(camera='forward', target_class='gate', axes='lat',
    ...
    assert mongla._heading_lock is not None
    assert abs(mongla._heading_lock.target_deg - 90.0) < 0.5
...
def test_lat_only_align_no_lock_still_releases_ch4():
    ...
    """
    mongla, pix, hb = _make(armed=True)         # NO lock_heading
...
def test_vision_move_always_releases_ch4():
    ...
    mongla, pix, hb = _make(armed=True)         # NO lock
    pix.rc.clear(); pix.translations.clear()

    mongla.vision_move(camera='forward', target_class='gate', fwd_fill=95.0,
...
def test_deferred_lock_then_yaw_align_first_command():
    ...
    """
    mongla, pix, hb = _make(armed=False)        # start DISARMED
    mongla.lock_heading(target=0.0, timeout=30.0)
    assert mongla._lock_deferred is True
    assert mongla._heading_lock.is_suspended is True

    mongla.arm()
    assert mongla._lock_deferred is True          # arm alone doesn't activate

    # First armed actuating command is a yaw-align.
    mongla.vision_align(camera='forward', target_class='gate', axes='yaw',
                        err_px=40.0, duration=0.2, gain=30.0, kp_yaw=60.0)

    assert mongla._lock_deferred is False         # activated
    assert mongla._heading_lock is not None       # not stranded
    assert mongla._heading_lock.is_suspended is False  # resumed after the loop
    assert abs(mongla._heading_lock.target_deg - 10.0) < 0.5  # final heading
