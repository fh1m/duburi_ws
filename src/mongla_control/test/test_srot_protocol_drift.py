...
def test_every_firmware_flight_mode_has_a_name():
    ...
    unmapped. An unmapped custom_mode surfaces on /mongla/state as 'UNKNOWN(100)',
...
def test_esc_status_291_is_absent_from_our_dialect():
    """Pins the reason /mongla/esc_rpm cannot be built on ESC_STATUS today.
    ...
    """
    from mongla_control.fc import srot_fc            # noqa: F401  -- sets MAVLINK20
...
def test_bondor_mirrors_the_same_payload_function_enum():
    ...
    (its gate is `npm run typecheck`, which cannot see the firmware). mongla_ws
...
def test_the_companion_link_baud_matches_the_firmware():
    ...
    """
    from mongla_manager.connection_config import SROT_BAUD
