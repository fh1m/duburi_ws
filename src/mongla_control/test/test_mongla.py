"""Integration-ish tests for the Mongla facade.
...
effects) is exercised through the real Mongla class.
"""
...
from mongla_control.errors import NotArmedError
...
class FakePixhawk:
    ...
    Mongla reaches for a method we forgot to implement here.
    """
...
def mongla():
    pixhawk = FakePixhawk()
    log     = ThrottleLogger(logging.getLogger('test.mongla'))
    return Mongla(pixhawk, log)


def test_arm_returns_move_result(mongla):
    result = mongla.arm()
...
def test_set_mode_returns_move_result(mongla):
    result = mongla.set_mode('MANUAL')
    assert isinstance(result, Move.Result)
    assert result.success is True


def test_stop_sends_neutral_and_returns_result(mongla):
    result = mongla.stop(settle_time=0)
    assert ('send_neutral',) in mongla.pixhawk.calls
    assert result.success is True


def test_pause_releases_then_neutralises(mongla):
    result = mongla.pause(duration=0.05)
    kinds = [call[0] for call in mongla.pixhawk.calls]
...
def test_move_forward_short_burst_returns_depth(mongla, monkeypatch):
    # Hot loop: shrink any sleep so the test doesn't actually wait.
    monkeypatch.setattr(time, 'sleep', lambda *_: None)
    result = mongla.move_forward(duration=0.05, gain=50.0)
    assert isinstance(result, Move.Result)
    assert result.success is True
    assert math.isclose(result.final_value, -0.5, abs_tol=0.01)


def test_arc_dispatches_to_motion_forward(mongla, monkeypatch):
    monkeypatch.setattr(time, 'sleep', lambda *_: None)
    result = mongla.arc(duration=0.05, gain=50.0, target_yaw=30.0)
    assert isinstance(result, Move.Result)
    assert result.success is True
    # arc emits packets with both forward + yaw -- this is the
    # single-packet contract that makes curved motion possible.
    arc_packets = [
        c for c in mongla.pixhawk.calls
        if c[0] == 'send_rc_override' and 'forward' in c[1] and 'yaw' in c[1]]
    assert arc_packets, 'arc must emit Ch5 + Ch4 in the same packet'


def test_lock_heading_starts_thread_and_unlock_stops(mongla, monkeypatch):
    monkeypatch.setattr(time, 'sleep', lambda *_: None)
    assert mongla._heading_lock is None
    mongla.lock_heading(target=90.0, timeout=2.0)
    assert mongla._heading_lock is not None
    mongla.unlock_heading()
    assert mongla._heading_lock is None


def test_lock_heading_target_zero_locks_current_heading(mongla, monkeypatch):
    """target=0.0 (rosidl unset) means "lock at current heading"."""
    monkeypatch.setattr(time, 'sleep', lambda *_: None)
    result = mongla.lock_heading(target=0.0, timeout=2.0)
...
def test_surface_does_not_deadlock(mongla, monkeypatch):
...
def test_disarm_stops_active_heading_lock(mongla, monkeypatch):
    ...
    mongla.disarm()
    assert mongla._heading_lock is None
    assert mongla.pixhawk.is_armed() is False


def test_set_depth_engages_alt_hold(mongla, monkeypatch):
    """set_depth must guarantee ALT_HOLD is engaged before driving."""
    monkeypatch.setattr(time, 'sleep', lambda *_: None)
    mongla.pixhawk._mode = 'MANUAL'
    mongla.set_depth(target=-1.0, timeout=2.0)
    assert mongla.pixhawk._mode == 'ALT_HOLD', (
...
def test_quick_settle_skips_pause_on_same_axes(monkeypatch):
    ...
    pixhawk = FakePixhawk()
    log     = ThrottleLogger(logging.getLogger('test.mongla.quick'))
    d       = Mongla(pixhawk, log, quick_settle=True)
...
def test_quick_settle_off_by_default_keeps_pause(monkeypatch):
    ...
    pixhawk = FakePixhawk()
    log     = ThrottleLogger(logging.getLogger('test.mongla.classic'))
    d       = Mongla(pixhawk, log)        # quick_settle defaults to False
    ...
    assert 0.6 in sleeps, (
        'default Mongla() must always settle 0.6 s before each command')
...
def test_quick_settle_pauses_when_axes_differ(monkeypatch):
    ...
    pixhawk = FakePixhawk()
    log     = ThrottleLogger(logging.getLogger('test.mongla.qs.diff'))
    d       = Mongla(pixhawk, log, quick_settle=True)
...
def test_motion_uses_translation_when_lock_active(mongla, monkeypatch):
    ...
    """
    monkeypatch.setattr(time, 'sleep', lambda *_: None)
    mongla.lock_heading(target=0.0, timeout=2.0)
    mongla.pixhawk.calls.clear()
    mongla.move_forward(duration=0.05, gain=50.0)
    ...
    translation_calls = [c for c in mongla.pixhawk.calls
...
def test_motion_verb_raises_not_armed_when_disarmed(monkeypatch):
    ...
    log     = ThrottleLogger(logging.getLogger('test.mongla.unarm'))
    d       = Mongla(pixhawk, log)
...
def test_unarm_safe_verbs_succeed_when_disarmed(monkeypatch):
    ...
    monkeypatch.setattr(time, 'sleep', lambda *_: None)
    pixhawk = FakePixhawk(armed=False)
    log     = ThrottleLogger(logging.getLogger('test.mongla.unarm.safe'))
    d       = Mongla(pixhawk, log)
...
def test_motion_succeeds_after_arm(monkeypatch):
    ...
    monkeypatch.setattr(time, 'sleep', lambda *_: None)
    pixhawk = FakePixhawk(armed=False)
    log     = ThrottleLogger(logging.getLogger('test.mongla.arm.then.move'))
    d       = Mongla(pixhawk, log)
...
# --------------------------------------------------------------------------- #
def test_report_vision_then_telemetry_fresh(mongla):
    mongla.report_vision(-42.0, 8.0)
    assert mongla.vision_telemetry() == (-42.0, 8.0)


def test_vision_telemetry_stale_returns_none(mongla):
    mongla.report_vision(10.0, 20.0)
    # A tiny freshness window must elapse -> stale -> None (no live verb).
    time.sleep(0.05)
    assert mongla.vision_telemetry(fresh_s=0.001) is None


def test_vision_telemetry_none_before_any_report(mongla):
    assert mongla.vision_telemetry() is None


def test_make_result_sets_vision_fields(mongla):
    r = mongla._make_result(True, 'x', final_value=0.0, error_value=12.0,
...
def test_make_result_defaults_nan_for_non_vision(mongla):
    r = mongla._make_result(True, 'arm', final_value=-0.5)
...
def test_calibrate_depth_success_when_surfaced_and_disarmed(monkeypatch):
    ...
    mongla  = Mongla(pixhawk, ThrottleLogger(logging.getLogger('test.baro')))
    r = mongla.calibrate_depth(settle_s=0.0)
...
def _mongla_with_distance(ds, armed=True):
    pixhawk = FakePixhawk(armed=armed)
    d = Mongla(pixhawk, ThrottleLogger(logging.getLogger('test.dist')),
