...
to the loop and to the DSL -- and the `--tool` flag still died with
    ...
    mongla vision_align: error: argument --tool: invalid _float_or_head value: 'torpedo'
...
from mongla_control.commands import BOOL_FIELDS, COMMANDS, STRING_FIELDS

_ACTION = (pathlib.Path(__file__).resolve().parents[2] / 'mongla_interfaces'
           / 'action' / 'Move.action')
