...
from mongla_control.fc.srot_fc import SrotFC, _build_params, MOVE_VERBS
...
from mongla_control.fc.base import (SUCCEEDED, PREEMPTED, FAILED, DENIED,
                                    TIMEOUT, ABORTED)
...
def test_telemetry_decodes_attitude_depth_and_mode():
    ...
    # NEGATIVE below the surface -- the stack-wide convention (MonglaState.msg,
...
def test_ack_deadline_tracks_the_leg_not_the_60s_board_default():
    ...
    import mongla_control.fc.srot_fc as mod
    from mongla_control.fc.srot_fc import _ack_budget_s
...
def test_stop_verb_is_a_single_bare_stop():
    """`mongla stop` is now ONE MOVE_STOP -- the board brakes it (fw rev 2).
...
def _assert_srot_ported(names):
    ...
    pkg = Path(__file__).resolve().parents[1] / 'mongla_control'
    ...
    *** THE "EVERY REV IS ADDITIVE" CLAIM ABOVE EXPIRED AT REV 10. ***  It held for
    ...
    later revisions fail it:
      ...
      rev 13  SROT_MOVE REQUIRES ARMED. Below it, a move sent while disarmed runs its
              ...
              The firmware's own note names mongla_ws as the consumer that would
...
def test_get_battery_is_pinned_to_the_main_pack_not_whatever_arrived_last():
    ...
    so /mongla/state's battery reading was a coin flip on which battery it meant."""
    fc = _fc()
...
def _facade_fc_calls():
    ...
    root = _pathlib.Path(__file__).resolve().parents[1] / 'mongla_control'
    found: dict = {}
    for fname in ('mongla.py', 'vision_verbs.py'):
...
def test_every_facade_fc_call_resolves_on_srot_or_its_verb_is_handled():
    ...
    from mongla_control.fc.srot_fc import MOVE_VERBS, UNSUPPORTED_VERBS
