...
READ AS TEXT, NEVER IMPORTED. In a worktree `import mongla_control` resolves
...
PROTO = ROOT / 'mongla_control' / 'mongla_control' / 'fc' / 'srot_protocol.py'
...
def test_the_fov_blocker_is_not_still_asserted_as_open():
    ...
    """
    cal = (ROOT / 'mongla_vision' / 'config' / 'calibration')
...
def test_the_health_states_it_documents_are_the_states_that_SHIP():
    ...
    """
    src = (ROOT / 'mongla_manager' / 'mongla_manager' / 'health.py').read_text()
...
def test_the_documented_loop_rate_matches_the_shipped_one():
    ...
    rates = (ROOT / 'mongla_control' / 'mongla_control'
