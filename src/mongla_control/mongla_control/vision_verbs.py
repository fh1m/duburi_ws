#!/usr/bin/env python3
"""Vision verbs for the Mongla facade -- exactly two, pixel-native.
...
Mixed into ``Mongla`` via multiple inheritance; uses only the base
...
class VisionVerbs:
    """Camera-driven verbs for the Mongla facade. Never instantiated alone."""
    ...
    def _warn_tool_geometry(self, tool, vstate):
        ...
        try:
            from mongla_vision.tool_geometry import is_measured, known_tools
        except ImportError:
            return
            ...
            try:
                with self._suspend_heading_lock() if touches_yaw else nullcontext():
                    ...
                    outcome = align_loop(
                        ...
                        # layer allowed to import mongla_vision -- the geometry
                        # table lives there and mongla_control must not depend
