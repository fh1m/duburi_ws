...
# MUST precede the pymavlink import, exactly as `mongla_control.pixhawk` does.
#
...
# `UNKNOWN_11030` and `/mongla/esc_rpm` reads nothing. Silently: no error, no
...
#
# It happened to work only because `mongla_control/__init__` reaches `pixhawk.py`
...
_DEPTH_ARM_TAIL = (
    ...
    'this is a large zero offset, not a dead sensor -- run `mongla calibrate_depth`, '
    'then re-check with `ros2 run mongla_manager connect`')
...
class SrotFC(FlightController):
    ...
    # ------------------------------------------------------------------ #
    #  move() -- the mongla-verb -> SROT_MOVE mapping + ACK state machine #
    # ------------------------------------------------------------------ #
    ...
    def _relay_move_ack(self, verb, on_progress, abort_fn, budget) -> MoveResult:
        ...
        self._log_warn(
            ...
            f'`ros2 run mongla_manager connect` tells them apart.')
        return MoveResult(TIMEOUT, f'{verb}: no terminal ACK within {budget:.0f}s (stall) -- the board took the '
                ...
                f'alive (`ros2 run mongla_manager connect`) and that the board is '
    ...
    def thruster_health(self):
        ...
        approval. `mongla_ws` uses it to refuse a PAYLOAD run (where a dead
    ...
    def telemetry(self) -> Telemetry:
        ...
        # MonglaState.msg documents. VFR_HUD.alt is already in that convention
        ...
        # confident 0.00 m on /mongla/state, in [STATE], and into every depth guard
        ...
        # for our 8 thrusters. `/mongla/esc_rpm` is built on that fallback below (see
    ...
    def get_attitude(self):
        ...
        DEPTH SIGN: this stack's convention -- MonglaState.msg, set_depth's input,
        ...
        # failure for another. `MonglaState.msg` documents NaN as the absence
    ...
    def get_battery(self):
        ...
        /mongla/state is unchanged. Pinned to id 0 rather than 'whatever arrived
    ...
    def send_landing_target(self, bearing, *, target_num: int = 0,
        ...
        `bearing` is a `mongla_control.bearing.Bearing`. One message per frame
...
# ---------------------------------------------------------------------- #
#  The verb table -- mongla Move verb -> SROT_MOVE (p1..p5).             #
...
def _depth_to_dive(kw) -> float:
    """mongla set_depth target is NEGATIVE metres (below surface); SROT DIVE p2 is a
...
def _build_params(verb: str, kw: dict):
    ...
    # NOTE: 'arc' is deliberately NOT mapped. mongla's arc holds an ABSOLUTE target
...
# imported by `mongla.py`. Removing an entry re-arms whatever that verb touches.
...
# `ros2 run mongla_manager connect`. The previous design routed a "mongla channel"
...
class SrotPayload:
    ...
    `port_path`) so the Mongla facade is unchanged; there is NO separate USB
