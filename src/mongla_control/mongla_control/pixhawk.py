...
else in `mongla_control` and `mongla_manager` talks to the autopilot
...
Activate with ``ros2 run mongla_manager start --ros-args -p
...
class Pixhawk:
    ...
    def _log_mavlink(self, body):
        ...
        ``tracing.current_tag()`` (a contextvar the Mongla facade opens
    ...
    def get_mode(self):
        ...
        # hard per-command preconditions (mongla.py `_command_scope` /
