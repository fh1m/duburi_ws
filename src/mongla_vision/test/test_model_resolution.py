...
from mongla_vision.detection.yolo import _resolve_model_path   # noqa: E402
...
def tree(tmp_path, monkeypatch):
    ...
    models.mkdir()
    monkeypatch.setattr('mongla_vision.detection.yolo._find_src_models_dir',
                        lambda: models)
    monkeypatch.delenv('MONGLA_HEF_DIR', raising=False)
    return models
...
def test_MONGLA_HEF_DIR_is_read(tree, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv('MONGLA_HEF_DIR', str(hefs))
    assert _resolve_model_path('gate') == str(hefs / 'gate.hef')


def test_MONGLA_HEF_DIR_beats_a_stale_stem_in_the_tree(tree, tmp_path, monkeypatch):
...
def test_an_unset_or_empty_env_does_not_change_a_dev_box(tree, monkeypatch):
    _touch(tree, 'gate.pt')
    for val in ('', '   '):
        monkeypatch.setenv('MONGLA_HEF_DIR', val)
        assert _resolve_model_path('gate').endswith('gate.pt')
...
def test_a_tilde_in_the_env_is_expanded(tree, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv('MONGLA_HEF_DIR', '~/hailo_models')
    assert _resolve_model_path('gate').endswith('hailo_models/gate.hef')
