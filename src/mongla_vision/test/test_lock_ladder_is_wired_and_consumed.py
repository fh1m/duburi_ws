...
def test_the_anchor_model_follows_MONGLA_HEF_DIR():
    ...
    # `'MONGLA_HEF_DIR' in window` and passed against code where the lookup
    # had been replaced by '' -- because the comment ABOVE it still said
    # MONGLA_HEF_DIR. The test was reading prose. Injection is what exposed
    # it: the defect went in and nothing failed.
    assert "os.environ.get('MONGLA_HEF_DIR'" in window, (
        'the anchor does not READ MONGLA_HEF_DIR (a comment mentioning it is '
