...
publish a synthetic `/mongla/state` so `flow_node` has a height on a bench
...
def _state_publishers():
    """Tools that construct a MonglaState to publish."""
    if not _TOOLS.is_dir():
        pytest.skip('tools/ not present')
    out = []
    for py in sorted(_TOOLS.glob('*.py')):
        src = py.read_text()
        if 'MonglaState()' in src and '/mongla/state' in src:
...
def test_a_synthetic_state_never_asserts_a_yaw_it_does_not_measure():
    ...
    assert tools, ('no tool publishes a synthetic /mongla/state any more -- '
    ...
    assert not bad, (
        'a bench tool publishes a CONCRETE yaw into /mongla/state:\n  ' +
        '\n  '.join(bad) +
