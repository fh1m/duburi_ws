...
`mongla_vision/config.py` holds the dict `get_profile` resolves against.
...
from mongla_vision.config import CAMERA_PROFILES        # noqa: E402
...
def test_every_source_has_a_builder():
    ...
    from mongla_vision.factory import BUILDERS
    for name, prof in CAMERA_PROFILES.items():
...
def test_device_path_in_a_profile_reaches_the_builder(monkeypatch):
    ...
    import mongla_vision.factory as F
    ...
    F.make_camera_from_profile(
        {'source': 'v4l2', 'device_path': '/dev/mongla_cam_forward',
         'width': 640, 'height': 360, 'fps': 60, 'name': 'f'})
    assert seen.get('device') == '/dev/mongla_cam_forward', seen
...
def test_the_pi_profiles_name_a_symlink_this_OS_creates(name):
    ...
    `/dev/mongla_cam_*` instead, and the rule earned itself immediately: video0
    ...
    assert dev.startswith('/dev/mongla_cam_'), dev
...
def test_fourcc_in_a_profile_reaches_the_builder():
    ...
    try:
        F.make_camera_from_profile(
            {'source': 'v4l2', 'device_path': '/dev/mongla_cam_downward',
...
def test_the_webcam_fallback_carries_the_format_too():
    ...
    import mongla_vision.cameras.webcam as wc
