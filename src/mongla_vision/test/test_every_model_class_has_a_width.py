    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with open(path, 'w') as fh:
            yaml.safe_dump({'robosub': {'gate': {'width_m': 1.82,
                                                 'boxes': 'whole-gate'}}}, fh)
        monkeypatch.setenv('MONGLA_TARGET_GEOMETRY', path)
        tg._CACHE.clear()
...
def test_a_directory_works_as_well_as_a_file(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as d:
        with open(os.path.join(d, 'target_geometry.yaml'), 'w') as fh:
            yaml.safe_dump({'robosub': {'gate': {'width_m': 1.82}}}, fh)
        monkeypatch.setenv('MONGLA_TARGET_GEOMETRY', d)
        tg._CACHE.clear()
        assert tg.width_for('gate') == pytest.approx(1.82)
    tg._CACHE.clear()
...
def test_the_no_env_var_path_is_the_one_the_deck_warning_names(monkeypatch):
    """`~/.mongla/target_geometry.yaml` had never once been exercised.

    Every other override test goes through MONGLA_TARGET_GEOMETRY, and the
    ...
    """
    tg = _geometry()
    monkeypatch.delenv('MONGLA_TARGET_GEOMETRY', raising=False)
    with tempfile.TemporaryDirectory() as home:
        os.makedirs(os.path.join(home, '.mongla'))
        with open(os.path.join(home, '.mongla', 'target_geometry.yaml'), 'w') as fh:
