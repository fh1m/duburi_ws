...
def _config_yamls():
    return sorted(_ROOT.glob('mongla_*/config/*.yaml'))


def _launch_sources():
    return sorted(_ROOT.glob('mongla_*/launch/*.py'))


def _package_sources():
    """Python that could read a config directly, not via a launch parameter."""
    return sorted(_ROOT.glob('mongla_*/mongla_*/**/*.py'))
...
def test_a_file_the_operator_loads_says_how():
    ...
    from the inert ones.
    """
    p = _ROOT / 'mongla_manager' / 'config' / 'vision_tunables.yaml'
    if not p.exists():
        return
...
def test_modes_yaml_does_not_carry_the_swapped_network_table():
    ...
    """
    p = _ROOT / 'mongla_manager' / 'config' / 'modes.yaml'
    if not p.exists():
        return
    txt = p.read_text()
...
def test_sensors_yaml_does_not_present_the_unfitted_dvl_as_the_default():
    ...
    p = _ROOT / 'mongla_sensors' / 'config' / 'sensors.yaml'
    if not p.exists():
        return
    head = '\n'.join(p.read_text().splitlines()[:30])
