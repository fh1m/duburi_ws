...
What it tests (all topic-based, no model load):
  * /mongla/vision/<cam>/image_raw       -- frames per second
  * /mongla/vision/<cam>/camera_info     -- presence + image dims
  * /mongla/vision/<cam>/detections      -- frequency + class labels seen
...
ros2 run mongla_vision vision_check --camera laptop --duration 5
ros2 run mongla_vision vision_check --camera sim_front --require-class person
"""
...
def main(argv=None):
    ...
    parser.add_argument('--camera',         default='laptop',
                        help='camera namespace under /mongla/vision/ (default: laptop)')
    ...
    ns   = f'/mongla/vision/{args.camera}'
    ...
    # QoS reads as a broken camera. See `mongla_vision/qos.py`.
