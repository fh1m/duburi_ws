...
a single command:
    ...
    ros2 run mongla_vision vision_display --ros-args -p launch_pipeline:=true
...
from mongla_vision.config import CAMERA_PROFILES
from mongla_vision.detection.detector import Detection, largest
from mongla_vision.detection.messages import array_to_detections
from mongla_vision.draw_widgets import pil_text, pil_text_size

_WAIT_LOG_INTERVAL = 5.0
_WINDOW_NAME       = 'mongla  //  mission control'
...
def _start_pipeline(camera: str, model: str, classes: str,
    ...
    camera_proc = subprocess.Popen([
        'ros2', 'run', 'mongla_vision', 'camera_node',
        '--ros-args',
        '-p', f'name:={camera}',
        '-p', f'source:={source}',
    ])
    ...
    detector_proc = subprocess.Popen([
        'ros2', 'run', 'mongla_vision', 'detector_node',
        '--ros-args',
        '-p', f'camera:={camera}',
...
class VisionDisplayNode(Node):
    def __init__(self):
        ...
        raw_topic = f'/mongla/vision/{camera}/image_raw'
        det_topic = f'/mongla/vision/{camera}/detections'

        self.get_logger().info(f'[DISP ] subscribing {raw_topic} (full-rate) + {det_topic}')

        self._bridge = CvBridge()
        self._state: MonglaState | None = None
        ...
        # [dist_m, height_m, n_tracks, active] from /mongla/vision/downward/distance_debug.
        self._distance_dbg = None
        if self.get_parameter('hud_distance').get_parameter_value().bool_value:
            self.create_subscription(
                Float32MultiArray, '/mongla/vision/downward/distance_debug',
        ...
        # on /mongla/vision/active_camera when a verb / use_camera switches cameras,
        ...
        self.create_subscription(String, '/mongla/vision/active_camera',
    ...
    def _create_cam_subs(self, camera: str) -> list:
        raw_topic  = f'/mongla/vision/{camera}/image_raw'
        det_topic  = f'/mongla/vision/{camera}/detections'
        cls_topic  = f'/mongla/vision/{camera}/classes_filter'
        trk_topic  = f'/mongla/vision/{camera}/tracks'
        vr_topic   = f'/mongla/vision/{camera}/vis_range'
        vmap_topic = f'/mongla/vision/{camera}/vis_range_map'
        return [
