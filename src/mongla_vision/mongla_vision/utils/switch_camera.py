...
Examples
--------
ros2 run mongla_vision switch_camera downward     # downward infers, forward pauses
ros2 run mongla_vision switch_camera forward      # forward infers, downward pauses
ros2 run mongla_vision switch_camera --cameras forward,downward,side downward
...
def main(argv=None):
    ...
    rclpy.init()
    node = Node('mongla_switch_camera')
    try:
        ...
        target = f'mongla_detector_{args.camera}'
