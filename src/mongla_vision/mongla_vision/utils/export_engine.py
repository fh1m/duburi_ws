...
Usage::
    ...
    # one model
    ros2 run mongla_vision export_engine gate_flare_medium_100ep
    # several
    ros2 run mongla_vision export_engine gate_nano_100ep flare_medium_100ep
    # every .pt in src/mongla_vision/models/
    ros2 run mongla_vision export_engine --all
...
from mongla_vision.detection.yolo import _find_src_models_dir


def _models_dir() -> Path:
    d = _find_src_models_dir()
    if d is None:
        print("[EXPORT] could not locate src/mongla_vision/models/", file=sys.stderr)
        sys.exit(2)
    return d
...
def main(argv=None) -> int:
    ...
    ap.add_argument('--all', action='store_true',
                    help='export every .pt in src/mongla_vision/models/')
