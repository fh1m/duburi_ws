...
The vision cousin of `mongla_sensors.sensors_node`: one node, no inter-node
...
ros2 run mongla_vision vision_node --ros-args -p profile:=laptop

# Headless smoke test, log only (no debug image)
ros2 run mongla_vision vision_node --ros-args -p profile:=laptop -p publish_debug_image:=false

# Force CPU
ros2 run mongla_vision vision_node --ros-args -p profile:=laptop -p device:=cpu
"""
