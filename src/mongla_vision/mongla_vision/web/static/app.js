...
function videoUrl(cam) {
  return `http://${VIDEO_HOST}:${VIDEO_PORT}/stream` +
         `?topic=/mongla/vision/${cam}/image_debug&type=mjpeg&quality=80`;
}
...
function copySnippet(cam, d) {
  ...
  const lines = [
    `mongla.use_camera('${cam}')`,
    `mongla.set_conf(${conf})                       # per-target detection threshold`,
    `mongla.vision.align('${d.cls}', lat=0, yaw=0, err=30)   # centre it (live dx=${d.dx ?? '?'} dy=${d.dy ?? '?'} px, fill=${d.fill ?? '?'}%)`,
  ];
  if (typeof d.fill === 'number') {
    lines.push(`# mongla.vision.move('${d.cls}', fwd=${Math.min(90, d.fill + 20)}, mode='area')   # drive to standoff (now ${d.fill}%)`);
  }
  lines.push(`# mongla.calc_distance('start'); mongla.move_forward(duration=2, gain=40); m = mongla.calc_distance('stop')`);
