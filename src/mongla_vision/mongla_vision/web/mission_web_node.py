"""mission_web -- browser mission-control console for the Mongla vision stack.

One ROS2 node that:
  * subscribes the per-camera detection/data topics + /mongla/state + the latched
    /mongla/vision/active_camera, and polls the topic-less detector params
  ...
  * writes vision control through the SAME surface the mission DSL writes --
    SetParameters on /mongla_detector_<cam> + the latched active_camera publish +
...
auto-opens) with:  ros2 launch mongla_vision mission_web.launch.py
"""
...
def _resolve_static_dir() -> Path:
    ...
    try:
        ...
        candidates.append(
            Path(get_package_share_directory('mongla_vision')) / 'web' / 'static')
...
class MissionWebNode(Node):
    def __init__(self):
        super().__init__('mongla_mission_web')
        ...
        self.create_subscription(String, '/mongla/vision/active_camera',
                                 ...
                                 callback_group=self._cbg)
        self.create_subscription(MonglaState, '/mongla/state',
    ...
    def _state_cb(self, msg: MonglaState):
        import math
    ...
    def _poll_loop(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                names = set()
            for cam in self.cameras:
                present = f'mongla_detector_{cam}' in names
    ...
    def switch_active_camera(self, target: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                names = set()
            if f'mongla_detector_{target}' not in names:
            ...
            for other in active_camera_targets(target, _KNOWN_CAMERAS):
                if f'mongla_detector_{other}' in names:
