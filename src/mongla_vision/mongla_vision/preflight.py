...
ultralytics here:
  ...
  1. `/mongla/vision/<cam>/image_raw`     publishing at >= require_image_hz
  2. `/mongla/vision/<cam>/camera_info`   seen at least once (so VisionState
...
`require_detections=True` adds:
  ...
  3. `/mongla/vision/<cam>/detections`    publishing at >= 1 Hz
...
silent so they can `ros2 launch mongla_vision vision.launch.py camera:=<cam>` or
...
def _build_failure_reason(camera, counters, require_image_hz, require_detections):
    ...
    if not counters.info_seen:
        bits.append(f"camera_info silent on /mongla/vision/{camera}/camera_info")
    if not bits:
        bits.append(
            f"image_raw below {require_image_hz:.1f}Hz on "
            f"/mongla/vision/{camera}/image_raw")
    if require_detections:
        bits.append(f"detections silent on /mongla/vision/{camera}/detections")
    hint = (
        " -- start the vision pipeline first, e.g. "
        "`ros2 launch mongla_vision vision.launch.py camera:="
        f"{camera}`")
