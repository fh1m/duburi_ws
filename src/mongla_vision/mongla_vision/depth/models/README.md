...
cp /path/to/depth-anything-v2-small.onnx \
   src/mongla_vision/mongla_vision/depth/models/depth-anything-v2-small.onnx
...
```bash
ros2 run mongla_vision depth_estimator \
  --ros-args \
  -p camera:=forward \
  -p model_path:=$(pwd)/src/mongla_vision/mongla_vision/depth/models/depth-anything-v2-small.onnx \
