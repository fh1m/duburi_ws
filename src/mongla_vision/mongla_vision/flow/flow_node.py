...
from mongla_vision.flow.flow_velocity import flow_velocity
...
class FlowVelocityNode(Node):
    def __init__(self):
        ...
        self.create_subscription(Image, f'{ns}/image_raw', self._on_image,
                                 img_qos)
        self.create_subscription(Vector3Stamped, '/mongla/imu_rates',
                                 self._on_rates, 50)
        self.create_subscription(MonglaState, '/mongla/state', self._on_state,
                                 10)
    ...
    def _on_state(self, msg: MonglaState) -> None:
        ...
        The manager now stamps `/mongla/state` from the board's own clock, so
    ...
    def _read_the_floor(self, gray, t: float) -> None:
        ...
        if self._tile_m <= 0.0:
            return
        try:
            from mongla_localization.tile_grating import measure
    ...
    def _read_the_lane(self, gray) -> None:
        ...
        try:
            import cv2
            from mongla_localization.pool_lines import detect
        except Exception:                       # noqa: BLE001
