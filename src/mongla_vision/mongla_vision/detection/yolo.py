...
#
# Pool / competition custom models go in src/mongla_vision/models/ as .pt files
...
def _find_src_models_dir() -> Optional[Path]:
    """Locate src/mongla_vision/models/ from the installed or source path.
    ...
    if '/install/' in path_str:
        ...
        src_dir = ws_root / 'src' / 'mongla_vision' / 'models'
        if src_dir.is_dir():
            return src_dir
    # Pre-build source tree: __file__ is src/mongla_vision/mongla_vision/detection/yolo.py
...
def _resolve_model_path(name: str) -> str:
    ...
    Priority order:
      ...
      2. Source tree    — workspace ``src/mongla_vision/models/<name>.pt``
    ...
    # (built on the Jetson via `ros2 run mongla_vision export_engine`), so on a
    # dev box without one we transparently fall back to the .pt.
    # MONGLA_HEF_DIR FIRST, when it is set. `tools/pi_env.sh` has exported it
    ...
    hef_dir = os.environ.get('MONGLA_HEF_DIR', '').strip()
    if hef_dir:
    ...
    raise FileNotFoundError(
        ...
        + ('\n'.join(searched) + '\n' if searched else '')
        + f"To fix:\n"
          f"  1. Place {name}.pt in src/mongla_vision/models/ and rebuild:\n"
          f"       colcon build --packages-select mongla_vision\n"
...
class YoloDetector(Detector):
    ...
    def __init__(self, *, model_path='yolov11n', device='cuda:0',
        ...
        # All Mongla models are detectors. A TensorRT .engine carries no task
