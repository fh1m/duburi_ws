...
shared HUD viewer. Detector nodes are named ``mongla_detector_forward``
and ``mongla_detector_downward`` -- exactly the names the mission DSL
targets via ``mongla.set_model(... camera='forward')`` /
...
chunk calls ``mongla.resume_detector(<cam>)`` then ``pause_detector``.
...
Usage:
    ...
    # Headless pool day:
    ros2 launch mongla_vision vision_dual.launch.py viewer:=false
    ...
    ros2 launch mongla_vision vision_dual.launch.py paused:=false
...
def generate_launch_description():
    args = [
        DeclareLaunchArgument('fwd_device',   default_value='0',
                              description='/dev/videoN index for the forward camera — LAST-RESORT '
                                          'fallback, only used on a box with no /dev/mongla_cam_* '
        ...
        # the node auto-binds /dev/mongla_cam_<forward|downward> WHEN THAT SYMLINK EXISTS
        ...
        DeclareLaunchArgument(
            ...
            description='Subscribe this ROS image topic instead of a webcam. '
                        'Set by the simulator to /mongla/sim/front_camera/image_raw.'),
        DeclareLaunchArgument(
            'dwn_topic', default_value='',
            description='Subscribe this ROS image topic instead of a webcam. '
                        'Set by the simulator to /mongla/sim/bottom_camera/image_raw.'),
    ...
    def camera(profile: str, device_arg: str, video_arg: str, loop_arg: str,
        ...
        # /mongla/vision/<name>/* and the manager's _vision_state_for(<name>)
        ...
        return Node(
            package='mongla_vision', executable='camera_node',
            name=f'mongla_camera_{profile}', output='screen', ros_arguments=_QUIET,
            parameters=[{
                'profile': prof,
                'source':  src,
                'name':    profile,
                ...
                # /dev/mongla_cam_<profile> udev symlink WHEN IT EXISTS (Jetson),
    ...
    def detector(profile: str, model_arg: str, models_arg: str,
        ...
        return Node(
            package='mongla_vision', executable='detector_node',
            name=f'mongla_detector_{profile}', output='screen', ros_arguments=_QUIET,
            parameters=[{
                'camera':              profile,
    ...
    def tracker(profile: str, conf_arg: str) -> Node:
        return Node(
            package='mongla_vision', executable='tracker_node',
            name=f'mongla_tracker_{profile}', output='screen', ros_arguments=_QUIET,
    ...
    viewer = Node(
        ...
        name='mongla_display', output='screen',
