...
def _calib(name: str) -> str:
    ...
    """
    try:
        p = os.path.join(get_package_share_directory('mongla_vision'),
...
def generate_launch_description():
    ...
    # to `mongla_detector_dual` and ros2 warned about duplicate names. The
    # launcher's own default name is already `mongla_detector_dual`, and the two
    # detectors name themselves.
    detectors = Node(
        package='mongla_vision', executable='detector_dual_node',
    ...
    def tracker(camera_name: str, rate_arg: str) -> Node:
        return Node(
            ...
            name=f'mongla_tracker_{camera_name}', output='screen',
    ...
    def solver(camera_name: str) -> Node:
        ...
        """
        return Node(
            package='mongla_localization', executable='pnp_node',
            name=f'mongla_pnp_{camera_name}', output='screen',
            parameters=[{
                'camera': camera_name,
                'medium': LaunchConfiguration('medium'),
            }],
    ...
    def solver_near(camera_name: str) -> Node:
        ...
        return Node(
            package='mongla_localization', executable='pnp_node',
            name=f'mongla_pnp_near_{camera_name}', output='screen',
            parameters=[{
                'camera':        camera_name,
                'medium':        LaunchConfiguration('medium'),
                'variant':       'near',
    ...
    def pose_fuse(camera_name):
        ...
        """
        return Node(
            package='mongla_localization', executable='pose_fuse_node',
            name=f'mongla_pose_fuse_{camera_name}', output='screen',
            parameters=[{'camera': camera_name}],
            condition=IfCondition(LaunchConfiguration('lock')),
        )
    ...
    return LaunchDescription(args + [
        ...
        Node(package='mongla_vision', executable='flow_node',
             name='mongla_flow_velocity', output='screen',
             parameters=[{
                 'camera':       'downward',
                 'medium':       LaunchConfiguration('medium'),
        ...
        pose_fuse('forward'),
        pose_fuse('downward'),
        Node(package='mongla_vision', executable='vision_display',
