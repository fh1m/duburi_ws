...
node ``mongla_detector_<camera>`` so the mission DSL
(``mongla.set_model(... camera='forward')`` etc.) and
``mongla.vision.*`` verbs reach it with no node-name guessing.

Start it AFTER the control stack (``mongla start`` / bringup), then run
vision missions.
...
Usage:
    ...
    ros2 launch mongla_vision vision.launch.py video_file:=/tmp/pool_run.mp4 classes:=gate

    # Gazebo / re-published image topic:
    ros2 launch mongla_vision vision.launch.py camera:=sim_front \\
        topic:=/mongla/sim/front_camera/image_raw

    # Start paused (resume per task to save GPU):
    ros2 launch mongla_vision vision.launch.py camera:=forward paused:=true

Nodes: mongla_camera_<camera>, mongla_detector_<camera>,
       mongla_tracker_<camera>, mongla_image_view (viewer).
...
def generate_launch_description():
    args = [
        ...
        # (/mongla/vision/<camera>/...) and the nodes (mongla_detector_<camera>)
        ...
        # CAMERA_PROFILES entry (mongla_vision/config.py -- the LOADED copy;
        ...
        DeclareLaunchArgument('camera',        default_value='forward',
                              description='Camera profile (forward|downward|sim_front|laptop|...). '
                                          'Drives node names: mongla_detector_<camera>.'),
        ...
        # Leave this EMPTY and the node auto-binds /dev/mongla_cam_<camera> WHEN THAT
        ...
        DeclareLaunchArgument('device_path',   default_value='',
                              description='override device (empty = auto /dev/mongla_cam_<camera> '
    ...
    # republishes to /mongla/vision/<camera>/image_raw so detector + viewer
    ...
    cam_node = ['mongla_camera_',  cam]
    det_node = ['mongla_detector_', cam]
    trk_node = ['mongla_tracker_',  cam]
    ...
    camera_node = Node(
        package='mongla_vision', executable='camera_node', name=cam_node,
        ...
        parameters=[{
            ...
            # /dev/mongla_cam_<camera> udev symlink WHEN IT EXISTS (Jetson), else
    ...
    detector_node = Node(
        package='mongla_vision', executable='detector_node', name=det_node,
    ...
    tracker_node = Node(
        package='mongla_vision', executable='tracker_node', name=trk_node,
        output='screen', ros_arguments=_QUIET, additional_env=_QUIET_ENV,
        parameters=[{
            'camera':             cam,
    ...
    depth_node = Node(
        package='mongla_vision', executable='depth_estimation_node',
        name=['mongla_depth_', cam], output='screen', ros_arguments=_QUIET,
    ...
    distance_node = Node(
        package='mongla_vision', executable='distance_estimation_node',
        name='mongla_distance_estimator', output='screen', ros_arguments=_QUIET,
        additional_env=_QUIET_ENV,
        parameters=[{
            'camera':          cam,
    ...
    lock_node = Node(
        ...
        name=['mongla_lock_', cam], output='screen',
        parameters=[{
            'camera':       cam,
    ...
    image_viewer = Node(
        package='mongla_vision', executable='vision_display',
        name='mongla_image_view', output='screen', additional_env=_QUIET_ENV,
        parameters=[{
            'camera':          cam,
