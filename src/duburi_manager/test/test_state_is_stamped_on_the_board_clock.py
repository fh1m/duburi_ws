"""`/duburi/state` carries board measurements; it must not carry host jitter.
...
happening. `ClockMap` was already fitted in this node for `/duburi/imu_rates`;
...
def test_both_state_publishers_use_the_helper():
    """There are TWO publishers of DuburiState (the fast path and the telemetry
...
def test_the_board_stamp_is_cached_ONLY_when_the_mapping_SUCCEEDED():
    ...
    under the name `_board_stamp`, and `/duburi/state` would then carry the
...
def test_flow_node_reads_the_depth_CAPTURE_stamp_not_arrival():
    ...
    """
    fn = (Path(__file__).resolve().parents[2] / 'duburi_vision'
          / 'duburi_vision' / 'flow' / 'flow_node.py').read_text()
    assert 'from duburi_vision.stamps import capture_monotonic' in fn, (
