...
⛔ AND the index must come from `duburi_vision.optics`, which exists to be "the
...
`duburi_control` must NOT depend on `duburi_vision`, so the CALLER supplies it
...
_SRC = (Path(__file__).resolve().parents[1] / 'duburi_manager'
...
def test_the_index_is_not_copied_into_duburi_control():
    """One truth, one copy. `bearing.py` must take the index, never define it."""
    b = (Path(__file__).resolve().parents[2] / 'duburi_control'
         / 'duburi_control' / 'bearing.py').read_text()
    assert '1.333' not in b, 'the refractive index was copied into duburi_control'
