...
topics (/duburi/vision/<camera>/...) and the nodes (duburi_detector_<camera>)
...
between `fwd_profile` and the fixed `duburi_camera_forward` node name.
...
_VISION_PKG = os.path.abspath(
    os.path.join(_PKG, '..', 'duburi_vision'))
...
def test_the_camera_ROLE_default_has_not_been_renamed():
    ...
    assert d.get('camera') == 'forward', (
        ...
        f"'forward'. That renames /duburi/vision/forward/* and "
        f"duburi_detector_forward, which the mission DSL's own default "
...
def test_bringup_actually_PASSES_the_profile_through():
    ...
    same class as the four configs in duburi_vision measured to reach
    nothing."""
