...
⛔ WHAT THIS CATCHES. `build_dubomini.sh` does not discover packages, it lists
...
def _built_packages() -> set:
    ...
    return set(re.findall(r'duburi_[a-z_]+', text)) & _src_packages()


def test_every_package_in_src_is_in_the_build_script():
    missing = _src_packages() - _built_packages()
    # duburi_interfaces is built by the interfaces-first step, which the script
