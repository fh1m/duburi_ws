"""SimDvlSource -- Gazebo's native DVL as a mongla_ws position source.
...
-------------------------
`mongla_sensors` sources carry no ROS dependency by design, and the Jetson has
no Gazebo install at all. Importing gz-transport at module scope would make
`import mongla_sensors.sources` fail on the vehicle. It is imported inside
