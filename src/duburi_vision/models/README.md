...
| `gate_flare_v1.pt`             | `gate_flare_v1.yaml`             | Custom RoboSub classes       |
...
Pass just the **stem name** (no path, no `.pt`):

```bash
# Launch:
ros2 launch duburi_vision cameras_.launch.py model:=gate_flare_v1

# Detector node directly:
ros2 run duburi_vision detector_node --ros-args -p model_path:=gate_flare_v1

# config/detector.yaml:
model_path: gate_flare_v1
```
