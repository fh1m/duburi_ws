...
in `duburi_vision/config/calibration/`, listed by the tool, and re-appliable
...
def test_the_shipped_calibrations_are_all_loadable_and_named_by_medium(g):
    ...
    assert lib, 'duburi_vision ships no calibrations'
    for e in lib:
...
def test_the_guide_never_imports_its_own_PACKAGE(g):
    ...
    reason. A `from duburi_vision...` anywhere in it -- including inside a
    function, which is where it slipped in -- re-enters
    `duburi_vision/__init__.py`, which imports `preflight`, which imports
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            bad += [n.name for n in node.names
                    if n.name.split('.')[0] == 'duburi_vision']
        elif isinstance(node, ast.ImportFrom):
            if (node.module or '').split('.')[0] == 'duburi_vision':
