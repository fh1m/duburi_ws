...
`import duburi_control` resolves to the MAIN workspace's `install/` tree -- a
...
# `duburi_vision` is imported from THIS worktree (inserted first); every
# CONSTANT below is still read as text. The distinction matters: the
# install-tree hazard bites on `duburi_control`, which has an installed copy
...
MV = 'duburi_control/duburi_control/motion_vision.py'
...
def test_track_buffer_covers_the_p99():
    ...
    src = (ROOT / 'duburi_vision' / 'config' / 'tracker.yaml').read_text()
