...
All functions are pure (frame in → annotated copy out).
"""
...
def _annotators():
    ...
    if _BOX_ANNOTATOR is None:
        ...
        _LBL_ANNOTATOR = sv.LabelAnnotator(text_thickness=1, text_scale=0.45)
...
def draw_detections(frame_bgr: np.ndarray,
    ...
    """Boxes + labels for every detection (no primary highlight)."""
    ...
    return lbl.annotate(scene=out, detections=sv_det, labels=labels)
...
def draw_vehicle_state(frame_bgr: np.ndarray, state) -> np.ndarray:
    """Top-right STATE panel. state: DuburiState or compatible object."""
    if frame_bgr is None:
        return frame_bgr
    out = frame_bgr.copy()
    h, w = out.shape[:2]
...
def render_all(frame_bgr: np.ndarray,
    ...
    Layer order (later layers paint on top):
      ...
      3. primary highlight (thick box + corners + crosshair + arrow)
      4. PERCEPTION panel    (top-left)
      5. ALIGNMENT panel     (bottom-left)
      6. STATE panel         (top-right, only if state provided)
      7. STALE banner        (only if not healthy)
    """
    if frame_bgr is None:
        return None
    ...
    if primary is not None:
        ...
        cv2.rectangle(out, (x1, y1), (x2, y2), C_ACCENT, 3, cv2.LINE_AA)
        _draw_corners(out, x1, y1, x2, y2, C_ACCENT, length=18, thickness=4)
    ...
    if tracking_on:
        trk_str = f'ON  n={n_tracks}'
        perc_rows.append(('TRK', trk_str, C_ACCENT))
    badge_border = C_OK if (healthy and detections) else (C_ERR if not healthy else C_BORDER)
