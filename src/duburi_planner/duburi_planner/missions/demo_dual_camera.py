...
LAUNCH (paused:=true is the norm -- detectors start idle, the mission resumes one):
  ros2 launch duburi_vision vision_dual.launch.py viewer:=true paused:=true \\
...
Run:  ros2 run duburi_planner mission demo_dual_camera
"""
...
def run(duburi, log=None):
    ...
    duburi.arm()
    duburi.set_depth(-1.2, timeout=30)
    ...
    duburi.set_conf(0.35, model='gate_rescue_repair', node=_FWD)
    duburi.set_conf(0.55, model='torpedo_blood_hole', node=_FWD)
    ...
    if duburi.wait_for('gate', timeout=10):
        duburi.vision.align('gate', yaw=0, lat=0, err=40, gain=30, duration=20)
    ...
    if duburi.wait_for('fire', timeout=10):
        if duburi.vision.align('fire', camera='downward', lat=0, fwd=0,
                               depth=35, fwd_mode='height', err=30, gain=25,
                               duration=25, max_depth_m=-2.0):
            duburi.pause(2.0)
            duburi.fire(3)          # dropper channel
    elif log:
    ...
    if duburi.wait_for('hole', timeout=10):
        # Standoff + centre + fire-from-lock (yaw dropped, heading_lock holds Ch4).
        duburi.vision.align('hole', lat=0, depth=0, fwd=55, fwd_mode='area',
