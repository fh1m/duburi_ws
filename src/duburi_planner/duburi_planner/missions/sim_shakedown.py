...
reading. In the simulator you can measure it exactly:
    ...
    ros2 topic echo /duburi/sim/ground_truth --once     # before and after
...
The SAUVC pool modelled in duburi_sim_worlds is **1.6 m deep** -- water surface
...
Override per run without editing this file:
    ...
    DUBURI_SHAKEDOWN_DEPTH=-0.8 ros2 run duburi_planner mission sim_shakedown

Run: ros2 run duburi_planner mission sim_shakedown
"""
import os

DEPTH_M      = float(os.environ.get('DUBURI_SHAKEDOWN_DEPTH', -1.2))
LEG_S        = float(os.environ.get('DUBURI_SHAKEDOWN_LEG_S', 5.0))
LEG_GAIN_PCT = float(os.environ.get('DUBURI_SHAKEDOWN_GAIN', 55.0))
SETTLE_S     = 4.0     # coast-down between legs; see note above about inertia
DEPTH_BUDGET_S = 45.0

# Pool geometry from duburi_sim_worlds/spec/arena.yaml. Advisory only -- the real
...
def run(duburi, log):
    ...
    duburi.set_depth(DEPTH_M, timeout=DEPTH_BUDGET_S)

    log(f'leg 1/2: forward {LEG_S:.0f}s @ {LEG_GAIN_PCT:.0f}%')
    duburi.move_forward(LEG_S, gain=LEG_GAIN_PCT)
    ...
    duburi.pause(SETTLE_S)

    log(f'leg 2/2: back {LEG_S:.0f}s @ {LEG_GAIN_PCT:.0f}% (retracing leg 1)')
    duburi.move_back(LEG_S, gain=LEG_GAIN_PCT)
    duburi.pause(SETTLE_S)
