...
from duburi_planner.resilience import Attempt, never_fails, retry, selector
...
def _full_mission_source():
    import pathlib
    return (pathlib.Path(__file__).resolve().parents[1] / 'duburi_planner'
...
def test_release_heading_and_stop_are_contained():
    ...
    assert 'never_fails(duburi.release_heading' in src
    assert 'never_fails(duburi.stop' in src
...
def test_disarm_is_NOT_contained():
    ...
    src = _full_mission_source()
    assert 'never_fails(duburi.disarm' not in src, \
        'disarm must raise, loudly, even from a finally'
    assert 'duburi.disarm()' in src
