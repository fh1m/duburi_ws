...
def test_a_good_fuse_sets_the_offset():
    ...
    got = DuburiMission.anchor_heading(fake, bearing_deg=0.0)
    assert got.ok
...
def test_unanchored_absolute_heading_is_the_relative_one():
    assert DuburiMission.absolute_heading(_fake(head=42.0)) == pytest.approx(42.0)


def test_anchored_absolute_heading_applies_the_offset():
    assert DuburiMission.absolute_heading(
...
def test_absolute_to_relative_is_the_identity_when_unanchored():
    ...
    assert DuburiMission.absolute_to_relative(_fake(), 90.0) == pytest.approx(90.0)


def test_absolute_to_relative_undoes_the_offset():
    fake = _fake(offset=150.0)
    assert DuburiMission.absolute_to_relative(fake, 180.0) == pytest.approx(30.0)


def test_a_round_trip_closes():
    fake = _fake(head=30.0, offset=150.0)
    world = DuburiMission.absolute_heading(fake)
    assert DuburiMission.absolute_to_relative(fake, world) == pytest.approx(30.0)
