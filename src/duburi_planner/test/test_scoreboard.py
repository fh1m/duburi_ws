...
folder (DUBURI_RUN_DIR, default ~/duburi_runs), named per mission+timestamp,
...
from duburi_planner import duburi_dsl
from duburi_planner.duburi_dsl import DuburiMission, _run_dir


def _board(entries):
    m = object.__new__(DuburiMission)
...
def test_auto_writes_into_run_dir_with_mission_name(tmp_path, monkeypatch):
    monkeypatch.setenv('DUBURI_RUN_DIR', str(tmp_path))
...
def test_run_dir_env_override_and_creation(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv('DUBURI_RUN_DIR', str(target))
...
def test_git_sha_never_raises(monkeypatch):
    ...
    monkeypatch.setattr(duburi_dsl.subprocess, 'run', _boom)
    assert duburi_dsl._git_sha() == ''
