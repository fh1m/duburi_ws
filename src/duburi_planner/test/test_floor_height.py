"""`duburi.floor_height()`: the floor's own height, or None -- never a stale one."""
import time
...
TOPIC = '/duburi/vision/downward/floor_height'
...
def _run(publish_age_s=None, max_age_s=1.0):
    ...
    try:
        ...
        DuburiMission.floor_height(m, timeout=0.05)          # subscribe
        ...
        return DuburiMission.floor_height(m, timeout=0.05, max_age_s=max_age_s)
    finally:
        node.destroy_node()
