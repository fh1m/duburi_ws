...
node both publishes a Detection2DArray + CameraInfo and runs the DuburiMission
...
import duburi_planner.duburi_dsl as dd                        # noqa: E402
...
from duburi_vision.detection.messages import (                # noqa: E402
...
def test_detected_sees_live_published_frame(ros_node):
    # The node both publishes /detections and hosts the DuburiMission subs.
    pub = ros_node.create_publisher(
        Detection2DArray, '/duburi/vision/forward/detections', _QOS)
    client = SimpleNamespace(node=ros_node)
    duburi = dd.DuburiMission(client, _Log(), camera='forward')
    ...
    got = _publish_until(ros_node, pub, arr, lambda: duburi.detected('gate'))
    ...
    assert duburi.detected('hole') is False
...
def test_where_uses_live_camera_info(ros_node):
    ...
    info_pub = ros_node.create_publisher(
        CameraInfo, '/duburi/vision/forward/camera_info', _QOS)
    client = SimpleNamespace(node=ros_node)
    duburi = dd.DuburiMission(client, _Log(), camera='forward')
    ...
    assert duburi.where_offset('gate') < 0.0   # left of centre
...
def test_wait_for_returns_true_on_live_frame(ros_node):
    ...
    assert duburi.wait_for('red_pipe', timeout=3.0) is True
    # A class never published times out to False.
    assert duburi.wait_for('octagon', timeout=0.3) is False
