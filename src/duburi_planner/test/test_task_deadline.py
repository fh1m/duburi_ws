"""`duburi.task(name, deadline_s=)`: cancel the goal in flight and abandon cleanly.
...
from duburi_planner.client import DuburiClient, TaskAbandoned, goal_uuid_hex
...
def _client(monkeypatch):
    monkeypatch.setattr('duburi_planner.client.ActionClient', lambda *a, **k: MagicMock())
...
def test_the_task_block_sets_restores_and_records():
    m = _dsl()
    with DuburiMission.task(m, 'gate', deadline_s=30.0):
...
def test_an_abandoned_task_is_recorded_and_re_raised():
    m = _dsl()
    with pytest.raises(TaskAbandoned):
        with DuburiMission.task(m, 'torpedo', deadline_s=1.0):
...
def test_a_nested_task_keeps_the_earlier_deadline():
    m = _dsl()
    with DuburiMission.task(m, 'outer', deadline_s=5.0):
        outer = m.client._task_deadline
        with DuburiMission.task(m, 'inner', deadline_s=60.0):
...
def test_no_deadline_and_no_budget_changes_nothing():
    m = _dsl()
    with DuburiMission.task(m, 'bins'):
        assert m.client._task_deadline is None
