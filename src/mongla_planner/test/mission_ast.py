...
MISSIONS = Path(__file__).resolve().parents[1] / 'mongla_planner' / 'missions'
...
def sidecar(stem):
    ...
    directory, the vehicle in `MONGLA_HEF_DIR` (~/hailo_models) beside the HEF.
    ...
    """
    hefdir = os.environ.get('MONGLA_HEF_DIR') or str(Path.home() / 'hailo_models')
    for d in (Path(hefdir),
              SRC_ROOT / 'mongla_vision' / 'models',
              SRC_ROOT.parent / 'install' / 'mongla_vision' / 'share'
              / 'mongla_vision' / 'models'):
        cand = d / f'{stem}.yaml'
        if cand.is_file():
            return cand
    return None
