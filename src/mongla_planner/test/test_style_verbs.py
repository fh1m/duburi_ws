...
# ---------------------------------------------------------------------------
# Path setup: make mongla_control and mongla_sensors importable without colcon
# ---------------------------------------------------------------------------
import os
_ws = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..')
sys.path.insert(0, os.path.join(_ws, 'src', 'mongla_sensors'))
sys.path.insert(0, os.path.join(_ws, 'src', 'mongla_control'))
...
class TestBNO085PitchRollParsing(unittest.TestCase):
    ...
    def _make_source_with_mock_serial(self, lines):
        ...
        from mongla_sensors.sources.bno085 import BNO085Source
        encoded = [l.encode() + b'\n' for l in lines]
        idx = {'i': 0}
...
# ===========================================================================
# Mongla facade: heading lock sends 'L' to BNO
# ===========================================================================

class TestHeadingLockSendsL(unittest.TestCase):

    def _make_mongla(self, yaw_source=None):
        """Return a Mongla instance with fully mocked Pixhawk."""
        ...
        mongla = Mongla(px, log=log, yaw_source=yaw_source)
        mongla._abort_event.clear()
        return mongla
    ...
    def test_lock_heading_calls_send_command_L_when_bno(self):
        ...
        mongla = self._make_mongla(yaw_source=bno)

        # Patch HeadingLock so we don't spin a real thread
        with patch('mongla_control.mongla.HeadingLock') as MockHL:
            mock_lock = MagicMock()
            MockHL.return_value = mock_lock
            mongla.lock_heading(45.0, timeout=10.0)
    ...
    def test_lock_heading_no_error_without_send_command(self):
        ...
        with patch('mongla_control.mongla.HeadingLock') as MockHL:
            ...
            mongla.lock_heading(0.0)   # must not AttributeError

    def test_lock_heading_no_error_without_yaw_source(self):
        mongla = self._make_mongla(yaw_source=None)
        with patch('mongla_control.mongla.HeadingLock') as MockHL:
            MockHL.return_value = MagicMock()
            mongla.lock_heading(0.0)   # must not raise
...
def _make_mongla_for_style():
    """Return (mongla, pixhawk_mock) with enough stubs for style verbs."""
    from mongla_control.mongla import Mongla
    from mongla_control.pixhawk import Pixhawk
...
class TestStyleRoll(unittest.TestCase):
    ...
    def test_enters_acro_mode(self):
        mongla, px = _make_mongla_for_style()
        ...
        with patch('mongla_control.mongla.hold_depth'):
            mongla.style_roll(gain=60.0, timeout=5.0)
    ...
    def test_aborts_if_param_set_fails(self):
        ...
        result = mongla.style_roll(gain=60.0, timeout=5.0)
    ...
    def test_uses_bno_read_roll_when_available(self):
        ...
        bno.read_yaw  = MagicMock(return_value=0.0)
        mongla.yaw_source = bno

        with patch('mongla_control.mongla.hold_depth'):
            result = mongla.style_roll(gain=60.0, timeout=10.0)
...
class TestStyleRollMultiFlip(unittest.TestCase):
    ...
    def test_two_flips_require_720_degrees(self):
        ...
        with patch('mongla_control.mongla.hold_depth'):
            result = mongla.style_roll(gain=60.0, timeout=60.0, flips=2)
    ...
    def test_three_flips_complete_1080_degrees(self):
        ...
        with patch('mongla_control.mongla.hold_depth'):
            result = mongla.style_roll(gain=60.0, timeout=120.0, flips=3)
    ...
    def test_predive_called_before_acro(self):
        ...
        mongla, px = _make_mongla_for_style()
        mongla._abort_event.set()   # exit loop immediately
        px.get_attitude.return_value = {'yaw': 0.0, 'roll': 0.0, 'pitch': 0.0, 'depth': -0.7}
        ...
        with patch('mongla_control.mongla.hold_depth', side_effect=mock_hold_depth):
            mongla.style_roll(gain=60.0, timeout=5.0, flips=1, headroom=1.0)
    ...
    def test_headroom_zero_skips_predive(self):
        ...
        with patch('mongla_control.mongla.hold_depth', side_effect=mock_hold_depth):
            mongla.style_roll(gain=60.0, timeout=5.0, flips=1, headroom=0.0)
    ...
    def test_per_flip_returns_to_origin_depth_not_headroom(self):
        ...
        with patch('mongla_control.mongla.hold_depth', side_effect=mock_hold_depth):
            result = mongla.style_roll(gain=60.0, timeout=60.0, flips=2, headroom=1.0)
    ...
    def test_surface_guard_aborts_flip(self):
        ...
        with patch('mongla_control.mongla.hold_depth', side_effect=mock_hold_depth):
            result = mongla.style_roll(gain=60.0, timeout=5.0, flips=1, headroom=1.0)
...
class TestStyleYaw(unittest.TestCase):
    ...
    def test_calls_yaw_motion_n_times(self):
        ...
        with patch('mongla_control.mongla.yaw_snap') as mock_yaw, \
             patch('mongla_control.mongla.yaw_glide'):
            mock_yaw.return_value = None
            mongla.style_yaw(flips=1, deg_per_step=90.0, settle=0.0)
    ...
    def test_two_flips_calls_yaw_eight_times(self):
        ...
        with patch('mongla_control.mongla.yaw_snap') as mock_yaw, \
             patch('mongla_control.mongla.yaw_glide'):
            mock_yaw.return_value = None
            mongla.style_yaw(flips=2, deg_per_step=90.0, settle=0.0)

        self.assertEqual(mock_yaw.call_count, 8)

    def test_no_mode_change_stays_alt_hold(self):
        mongla, px = _make_mongla_for_style()
        with patch('mongla_control.mongla.yaw_snap'):
            mongla.style_yaw(flips=1, deg_per_step=180.0, settle=0.0)
...
class TestHeartbeatReleasedBeforeCleanup(unittest.TestCase):
    ...
    def test_heartbeat_released_before_set_param_in_style_roll(self):
        ...
        mongla._release_heartbeat_for_lock = track_release
        mongla._hold_heartbeat_for_lock    = MagicMock()
...
# Skipped when mongla_interfaces is not yet installed (pre-colcon).
# ===========================================================================

import importlib.util as _iutil

@unittest.skipIf(
    _iutil.find_spec('mongla_interfaces') is None,
    'mongla_interfaces not installed — run: colcon build --packages-select mongla_interfaces',
)
class TestCommandFieldsMatchMoveGoal(unittest.TestCase):
    ...
    rebuild mongla_interfaces) or commands.py lists a wrong field name (fix it).
    """

    def test_every_command_field_exists_on_move_goal(self):
        from mongla_interfaces.action import Move        # type: ignore[import]
        ...
        self.assertFalse(
            ...
            + '\nAdd to Move.action and rebuild mongla_interfaces.',
        )
