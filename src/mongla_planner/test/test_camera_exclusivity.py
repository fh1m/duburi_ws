...
def _fake(live=None, present=True):
    m = MagicMock()
    m._live_camera = live
    m._KNOWN_CAMERAS = MonglaMission._KNOWN_CAMERAS
...
def _activate(fake, name):
    with patch('mongla_planner.mongla_dsl._time.sleep'):
        MonglaMission._activate_camera(fake, name)
...
def _resume(fake, camera):
    MonglaMission._resume_default_detector(fake, camera)
...
def test_presence_waits_for_discovery_when_asked():
    ...
    fake = _present_fake([[], ['mongla_detector_forward']])
    with patch('mongla_planner.mongla_dsl.rclpy.spin_once'):
        got = MonglaMission._detector_present(fake, 'forward', settle=1.0)
    assert got is True
...
def test_presence_without_settle_does_not_spin():
    ...
    fake = _present_fake([[]])
    with patch('mongla_planner.mongla_dsl.rclpy.spin_once') as spin:
        got = MonglaMission._detector_present(fake, 'forward')
    assert got is False
...
def test_presence_gives_up_at_the_budget():
    fake = _present_fake([[], [], [], [], [], [], [], [], [], [], [], []])
    with patch('mongla_planner.mongla_dsl.rclpy.spin_once'):
        got = MonglaMission._detector_present(fake, 'forward', settle=0.05)
    assert got is False
...
def test_camera_available_defaults_to_the_sticky_camera():
    fake = _avail_fake(True)
    MonglaMission.camera_available(fake)
