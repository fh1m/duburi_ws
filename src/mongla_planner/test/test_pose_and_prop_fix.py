...
class _Odom:
    def __init__(self, frame='pool', x=1.5, y=-2.0, z=1.2, yaw_deg=30.0):
        # /mongla/odom is NED: position.z is +depth (1.2 m down)
...
def test_a_pool_frame_pose_is_returned_and_decoded():
    got = MonglaMission.pose(_mission(_Odom(yaw_deg=30.0)))
...
def test_an_UNANCHORED_pose_is_withheld():
    ...
    assert MonglaMission.pose(m) is None
...
def test_any_frame_opts_in_to_the_unanchored_reading():
    ...
    got = MonglaMission.pose(_mission(_Odom(frame='odom')), any_frame=True)
...
def test_no_odom_at_all_is_None():
    ...
    try:
        ...
        assert MonglaMission.pose(m, timeout=0.05) is None
    finally:
        node.destroy_node()
...
def test_yaw_decodes_across_the_wrap():
    for deg in (0.0, 89.0, 179.0, -179.0, -90.0):
        got = MonglaMission.pose(_mission(_Odom(yaw_deg=deg)))
...
def test_range_is_the_pinhole_relation():
    ...
    px = f_px * w / z_true
    got = MonglaMission.range_to(_ranger([('gate', 0.0, 0.0, px, px, 0.9)],
                                         f_px=f_px), 'gate')
    assert got is not None
    z, sigma = got
...
def test_sigma_grows_with_the_SQUARE_of_range():
    ...
    if w <= 0.0:
        pytest.skip('no gate width in the table')
    f_px = 500.0
    near = MonglaMission.range_to(
        _ranger([('gate', 0, 0, f_px * w / 1.0, 10, 0.9)], f_px=f_px), 'gate')
    far = MonglaMission.range_to(
...
def test_the_LARGEST_box_sets_the_range_not_the_most_confident():
    ...
    z, _ = MonglaMission.range_to(_ranger(boxes), 'gate')
    z_small, _ = MonglaMission.range_to(
...
def test_an_unknown_class_has_no_range():
    assert MonglaMission.range_to(_ranger([]), 'not_a_real_prop_xyz') is None
...
def test_a_class_with_no_box_has_no_range():
    ...
    if width_for('gate') <= 0.0:
        pytest.skip('no gate width in the table')
    assert MonglaMission.range_to(_ranger([]), 'gate') is None
...
def test_one_prop_at_a_known_bearing_and_range_places_the_hull():
    ...
    got = MonglaMission.fix_from_prop(m, 'gate')
    assert got.ok
...
def test_the_range_sigma_is_carried_to_the_filter():
    ...
    MonglaMission.fix_from_prop(m, 'gate')
...
def test_a_prop_with_no_surveyed_position_is_refused():
    ...
    got = MonglaMission.fix_from_prop(_prop_fixer(course), 'gate')
...
def test_no_course_is_refused_by_name():
    got = MonglaMission.fix_from_prop(_prop_fixer(None), 'gate')
    assert not got.ok and 'use_course' in got.reason


def test_a_prop_not_in_the_course_is_refused():
    got = MonglaMission.fix_from_prop(_prop_fixer(_course(gate=(1.0, 1.0))),
                                      'slalom')
