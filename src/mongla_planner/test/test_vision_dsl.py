...
from mongla_planner.vision_dsl import _VisionDSL, VisionResult
from mongla_planner.client import MoveFailed
from mongla_control.motion_vision import ALIGNED, LOST, TIMEOUT, NO_CAMERA
...
def _mission(send):
    """Fake MonglaMission exposing only what _VisionDSL touches."""
    m = MagicMock()
    m.camera = 'forward'
    m.target = 'gate'
    m._send = send
...
def test_result_bool_still_works_for_back_compat():
    # The truthiness contract is unchanged: `if mongla.vision.align(...)`.
...
def _ready_mission(monkeypatch, dd, camera='forward'):
    """A MonglaMission with the pump stubbed + subscription faked (no ROS)."""
    m = dd.MonglaMission(MagicMock(), MagicMock(), camera=camera)
...
def test_detected_is_case_insensitive(monkeypatch):
    import mongla_planner.mongla_dsl as dd
...
def test_where_returns_label_from_cache(monkeypatch):
    import time as _t
    import mongla_planner.mongla_dsl as dd
    m = dd.MonglaMission(MagicMock(), MagicMock(), camera='forward')
    monkeypatch.setattr(m, '_pump_detections', lambda *a, **k: None)
    m._img_size['forward'] = (640.0, 480.0)
...
def test_detector_node_derivation():
    # Single naming rule: /mongla_detector_<camera>, camera defaults to the
    # mission's sticky camera; explicit node always wins.
    import mongla_planner.mongla_dsl as dd
    m = dd.MonglaMission(MagicMock(), MagicMock(), camera='forward')
    assert m._detector_node() == '/mongla_detector_forward'
    assert m._detector_node('downward') == '/mongla_detector_downward'
    assert m._detector_node(node='/mongla_detector_custom') == '/mongla_detector_custom'
...
def test_ensure_detector_aborts_loudly_when_node_absent():
    ...
    m = dd.MonglaMission(client, MagicMock(), camera='forward')
    with pytest.raises(RuntimeError, match='NOT FOUND'):
        m._ensure_detector('/mongla_detector_forward', timeout=0.01)
...
def test_set_detector_param_sends_typed_param(monkeypatch):
    ...
    m._set_detector_param('/mongla_detector_forward', 'conf', 0.6)
...
def test_align_ensures_and_programs_before_activating_camera():
    ...
    m = _mission(MagicMock(return_value=_result(ALIGNED)))
    m._detector_node.return_value = '/mongla_detector_downward'
...
# --------------------------------------------------------------------------- #
#  _wait_detector_warm logic (MonglaMission): fresh cache -> warm; stale/empty #
...
def test_wait_detector_warm_true_when_producing():
    ...
    assert MonglaMission._wait_detector_warm(fake, 'downward') is True
