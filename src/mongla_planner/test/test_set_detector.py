...
def test_one_parameter_is_written_to_the_named_node():
    fake = _fake()
    MonglaMission.set_detector(fake, masks=False)
    fake._set_detector_param.assert_called_once_with(
        '/mongla_detector_forward', 'masks', False)


def test_several_parameters_in_one_call():
    fake = _fake()
    MonglaMission.set_detector(fake, max_det=10, iou=0.5)
...
def test_an_empty_call_is_refused():
    ...
    with pytest.raises(ValueError):
        MonglaMission.set_detector(_fake())
...
def test_a_rejected_parameter_raises_rather_than_warning():
    ...
    with pytest.raises(RuntimeError):
        MonglaMission.set_detector(fake, nonsense=1)


def test_the_camera_selects_the_node():
    fake = _fake()
    MonglaMission.set_detector(fake, camera='downward', masks=True)
...
def _node_fake(camera='forward'):
    m = MagicMock()
    m.camera = camera
    m._NODE_SUFFIX = MonglaMission._NODE_SUFFIX
    m._subsystem_node = lambda kind, cam=None: MonglaMission._subsystem_node(
        m, kind, cam)
    return m


@pytest.mark.parametrize('kind,expect', [
    ('detector', '/mongla_detector_forward'),
    ('camera',   '/mongla_camera_forward'),
    ('tracker',  '/mongla_tracker_forward'),
    ('lock',     '/mongla_lock_forward'),
    ('pnp',      '/mongla_pnp_forward'),
    ('flow',     '/mongla_flow_velocity'),
    ('manager',  '/mongla_manager'),
])
def test_each_subsystem_resolves_to_its_node(kind, expect):
    assert MonglaMission._subsystem_node(_node_fake(), kind) == expect


def test_the_camera_selects_the_per_camera_instance():
    fake = _node_fake()
    assert MonglaMission._subsystem_node(
        fake, 'tracker', 'downward') == '/mongla_tracker_downward'


def test_single_instance_nodes_ignore_the_camera():
    fake = _node_fake()
    assert MonglaMission._subsystem_node(
        fake, 'flow', 'downward') == '/mongla_flow_velocity'


def test_an_unknown_subsystem_is_refused_by_name():
    with pytest.raises(ValueError):
        MonglaMission._subsystem_node(_node_fake(), 'sonar')


def test_set_node_writes_to_the_resolved_node():
    fake = _node_fake()
    MonglaMission.set_node(fake, 'tracker', coast_s=1.2)
    fake._set_node_param.assert_called_once_with(
        '/mongla_tracker_forward', 'coast_s', 1.2)


def test_set_node_refuses_an_empty_call():
    with pytest.raises(ValueError):
        MonglaMission.set_node(_node_fake(), 'tracker')
...
def test_side_on_measures_against_the_structure():
    ...
    assert MonglaMission.side_on(_side_fake(recs), 'rescue') == 'left'
...
def test_side_on_survives_an_off_axis_hull():
    ...
    assert MonglaMission.side_on(_side_fake(recs), 'repair') == 'right'


def test_side_on_is_unknown_without_the_structure():
    recs = [_rec('rescue', 250, 170, 40, 40)]
    assert MonglaMission.side_on(_side_fake(recs), 'rescue') == 'unknown'
...
def test_side_on_with_no_frame_is_unknown():
    assert MonglaMission.side_on(_side_fake([]), 'rescue') == 'unknown'
...
def test_a_duplicated_node_name_refuses_the_write():
    fake = _graph(['mongla_tracker_forward'] * 10 + ['mongla_manager'])
    with pytest.raises(RuntimeError) as exc:
        MonglaMission._refuse_duplicate_node(fake, '/mongla_tracker_forward')
    assert '10 nodes' in str(exc.value)


def test_a_single_node_is_fine():
    fake = _graph(['mongla_tracker_forward', 'mongla_manager'])
    MonglaMission._refuse_duplicate_node(fake, '/mongla_tracker_forward')
...
def test_the_leading_slash_is_not_what_makes_them_different():
    fake = _graph(['/mongla_tracker_forward', 'mongla_tracker_forward'])
    with pytest.raises(RuntimeError):
        MonglaMission._refuse_duplicate_node(fake, 'mongla_tracker_forward')
