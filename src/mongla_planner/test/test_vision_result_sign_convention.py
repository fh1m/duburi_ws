...
recovery inverted:
    ...
    if res.x_px < -30: mongla.move_right(1)     # target LEFT  -> went right
    elif res.x_px > 30: mongla.move_left(1)     # target RIGHT -> went left
...
from mongla_planner.vision_dsl import VisionResult


def _pairs(text):
    """[(comparison, verb)] for every `x_px <cmp> <n>: mongla.move_<dir>` line."""
    return re.findall(r'x_px\s*([<>])\s*-?\d+\s*:\s*mongla\.move_(right|left)', text)
