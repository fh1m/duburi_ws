...
Every discovered mission must:
  ...
  2. Expose a `run(mongla, log)` callable with exactly two required
...
def test_every_mission_accepts_runner_call():
    """The runner calls `fn(mongla, log)`. `log` may be optional
    (`run(mongla, log=None)`), but the two-positional call must bind and
    `mongla` must be required."""
    for name, fn in discover().items():
        ...
        try:
            ...
            raise AssertionError(
                f'mission {name!r} cannot be called as fn(mongla, log): '
                f'{sig} ({exc})')
        required = [
        ...
        assert 1 <= len(required) <= 2, (
            f'mission {name!r} signature must be `(mongla, log[=None])`, '
            f'got {sig}')
...
def test_one_broken_file_does_not_brick_registry(tmp_path, monkeypatch, capsys):
    ...
    (tmp_path / 'good_one.py').write_text('def run(mongla, log=None):\n    pass\n')
    (tmp_path / 'broken_one.py').write_text('raise RuntimeError("boom at import")\n')
    (tmp_path / 'also_good.py').write_text('def run(mongla, log=None):\n    pass\n')
    monkeypatch.setenv('MONGLA_MISSIONS_DIR', str(tmp_path))
...
def test_syntax_error_file_is_skipped_not_fatal(tmp_path, monkeypatch):
    from mongla_planner.missions import discover
    (tmp_path / 'fine.py').write_text('def run(mongla, log=None):\n    pass\n')
    (tmp_path / 'typo.py').write_text('def run(mongla log):\n    pass\n')  # SyntaxError
    monkeypatch.setenv('MONGLA_MISSIONS_DIR', str(tmp_path))
...
from mongla_planner.mission import _safe_shutdown
