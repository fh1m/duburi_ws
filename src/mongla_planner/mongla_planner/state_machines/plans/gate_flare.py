...
build_gate_flare_fsm() produces a StateMachine that works for both bodies:
  - Mongla 4.5  (has_dvl=True)  → passage moves use DVL distance
  - Mongla_agile 2.0 (has_dvl=False) → passage moves use timed thrust
...
GATE_FLARE_DEFAULTS: dict = {
    ...
    'pass_duration':  5.0,    # timed fallback (Mongla_agile)
    'pass_gain':      80,
    'return_dist_m':  1.5,
    'return_duration': 3.0,
    'find_timeout':   45.0,
...
def build_gate_flare_fsm(
    ...
    sm.add_state('DIVE',
                 SetDepthState(mongla, profile, depth_m=p['depth_m']),
    ...
    sm.add_state('FIND_GATE',
                 VisionSearchState(mongla, profile, target='gate',
    ...
    sm.add_state('HOME_GATE',
                 VisionAlignState(mongla, profile, target='gate',
                                  yaw=True, lat=True,
    ...
    sm.add_state('FIND_FLARE',
                 VisionSearchState(mongla, profile, target='flare',
                                   pattern='forward', gain=30,
                                   timeout=p['find_timeout']),
                 transitions={SUCCEED: 'HOME_FLARE', TIMEOUT: 'SURFACE', ABORT: 'SURFACE'})

    sm.add_state('HOME_FLARE',
                 VisionAlignState(mongla, profile, target='flare',
                                  yaw=True, depth=True,
                                  err=p['align_err_px'], gain=p['align_gain'],
                                  duration=p['align_duration']),
    ...
    sm.add_state('MOVE_FLARE',
                 VisionMoveState(mongla, profile, target='flare',
