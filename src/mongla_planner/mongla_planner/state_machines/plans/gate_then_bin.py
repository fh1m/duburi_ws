...
Camera is explicit per-state — no reliance on sticky mongla.camera between tasks.
"""
...
def build_gate_then_bin_fsm(
    ...
    sm.add_state('DIVE_GATE',
                 SetDepthState(mongla, profile, depth_m=p['gate_depth_m']),
    ...
    sm.add_state('PASS_GATE',
                 MoveForwardState(mongla, profile,
    ...
    sm.add_state('DIVE_BIN',
                 SetDepthState(mongla, profile, depth_m=p['bin_depth_m']),
    ...
    sm.add_state('LOCK_BIN',
                 VisionAlignState(mongla, profile,
                                  target='bin_a',
                                  camera='downward',      # explicit
                                  lat=0, fwd=0,           # downward: lat=left/right (Ch6),
                                                          # fwd = fore/aft SURGE (Ch5, image-Y)
                                  err=p['bin_err_px'],
    ...
    sm.add_state('DROP_BIN',
                 _DropStubState(mongla, profile),
                 transitions={SUCCEED: 'CONFIRM_DROP', ABORT: 'SURFACE'})

    sm.add_state('CONFIRM_DROP',
                 _ConfirmDropStubState(mongla, profile, camera='downward'),
...
class _DropStubState(MonglaState):
    ...
    def _run(self, bb):
        try:
            self.mongla.log.info('[DROP ] stub — actuator not wired yet')
        except Exception:
            pass
        self.mongla.pause(1.0)
        return SUCCEED


class _ConfirmDropStubState(MonglaState):
    ...
    def __init__(self, mongla, profile, camera: str = 'downward') -> None:
        super().__init__(mongla, profile, [SUCCEED])
        self._camera = camera
    ...
    def _run(self, bb):
        ...
        while time.monotonic() < deadline:
            if not self.mongla.detected('bin_a', camera=self._camera, stale_after=0.5):
                bb[BK.LAST_ERROR] = ''
                return SUCCEED
