...
Mongla (mongla_ws) runs on two bodies:
  • Mongla 4.5  — has DVL (Nortek Nucleus1000) + grabber/dropper/torpedo
  • Mongla_agile 2.0 — no DVL, no manipulators
...
Usage:
    profile = VehicleProfile.auto(node)      # probe /mongla_manager at runtime
    profile = VehicleProfile.mongla_heavy()      # hardcoded for Mongla 4.5
    profile = VehicleProfile.mongla_agile()      # hardcoded for Mongla_agile 2.0
"""
...
class VehicleProfile:
    ...
    @classmethod
    def mongla_heavy(cls) -> VehicleProfile:
        return cls(
            name='mongla_heavy',
            has_dvl=True,
    ...
    @classmethod
    def mongla_agile(cls) -> VehicleProfile:
        return cls(
            name='mongla_agile',
            has_dvl=False,
    ...
    def auto(cls, node) -> VehicleProfile:
        """Probe /mongla_manager ROS params to detect vehicle capability.

        Falls back to mongla_agile() profile on any failure — safe default is
        ...
        try:
            ...
            client = node.create_client(GetParameters, '/mongla_manager/get_parameters')
            if not client.wait_for_service(timeout_sec=_PROBE_TIMEOUT_S):
                node.get_logger().warn(
                    '[VehicleProfile.auto] /mongla_manager not reachable — '
                    'falling back to mongla_agile() profile (safe default)'
                )
                return cls.mongla_agile()
            ...
            try:
                node.get_logger().warn(
                    ...
                    'falling back to mongla_agile() profile'
                )
            except Exception:
                pass
            return cls.mongla_agile()
