"""vision_dsl -- mongla.vision.* closed-loop sub-namespace (two verbs).
...
class VisionResult:
    ...
    paradigm), e.g.::
        ...
        res = mongla.vision.align('gate', yaw=0, lat=0)
        if res:                       # ALIGNED
            ...                        # proceed
            ...
            if res.x_px > 30: mongla.move_right(1)
            elif res.x_px < -30: mongla.move_left(1)
    ...
    def __bool__(self) -> bool:           # ``if mongla.vision.align(...):``
        return self.ok
...
class _VisionDSL:
    """mongla.vision.* -- the closed-loop sub-namespace (align + move).
    ...
    def __init__(self, mission: 'MonglaMission'):
        self._dsl = mission
        ...
        *disable* -- to drop an axis, omit it (``lat`` / ``yaw`` /
        ...
        def _one_shot(remaining: float):
            return self._send(
                ...
                standoff_max_tilt_deg=float(standoff_max_tilt),
                tool=str(tool or ''),
    ...
    def _run_fallback(self, fallback: Callable, target: str,
        ...
        ``fallback(mongla)`` runs one short manoeuvre and returns.
        ``fallback(mongla, should_stop)`` may self-poll a longer sweep and
