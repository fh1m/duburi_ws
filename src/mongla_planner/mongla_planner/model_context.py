...
Usage in a mission::
    ...
    def run(mongla, log):
        ...
        mongla.models(strict=('slalom_combined', ['slalom_red', 'slalom_white']))
        mongla.vision.align(mongla.models.strict.slalom_red, yaw=0)   # validated
        # mongla.vision.align(mongla.models.strict.typo, yaw=0)        # → AttributeError
...
goal — no manual `mongla.use(...)` or `mongla.set_classes(...)` needed.
"""
    ...
    def __getitem__(self, idx: int) -> ClassRef:
        ...
        if not classes:
            raise IndexError(
                ...
                f"use attribute access: mongla.models.{name}.class_name")
        try:
...
class ModelRegistry:
    """Callable singleton attached to ``MonglaMission`` as ``mongla.models``.
    ...
    Access::
        ...
        mongla.models.gate.gate     # ClassRef('gate', 'gate')
        mongla.models.gate.flare    # ClassRef('gate', 'flare')

    Optional strict class list (validates at access time)::

        mongla.models(gate=('gate_flare_medium_100ep', ['gate', 'flare']))
        # mongla.models.gate.typo  → AttributeError

    Multiple calls accumulate (later registrations overwrite same alias).
    Returns self so ``m = mongla.models(...)`` still works if preferred.
    """
    ...
    def __call__(self, **kwargs) -> 'ModelRegistry':
        ...
        **kwargs : alias = spec
        ...
        Returns self so the call can be chained or assigned::
            ...
            mongla.models(gate='stem')          # register in-place
            m = mongla.models(gate='stem')      # m is mongla.models (same object)
        """
    ...
    def __getattr__(self, item: str):
        raise AttributeError(
            ...
            f"Call mongla.models({item}='model_stem') first.")
