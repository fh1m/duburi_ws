...
Run: ros2 run mongla_planner mission arc_demo
"""
...
def run(mongla, log):
    ...
    for i in range(2):
        ...
        if i == 0:
            mongla.yaw_right(TURN_DEG)
        else:
            mongla.yaw_left(TURN_DEG)
    ...
    mongla.move_forward(LEG_DURATION_S, gain=LEG_GAIN_PCT, settle=2.0)
    ...
    for target in ARC_TARGETS:
        ...
        mongla.arc(target, ARC_DURATION_S, gain=ARC_GAIN_PCT)
