...
Live tuning (between runs, no rebuild):
  ...
  ros2 param set /mongla_manager vision.kp_forward  180.0
...
def run(mongla, log):
    ...
    mongla.countdown(
        COUNTDOWN_S,
        message='Wire removed  --  Mongla is now autonomous. Good luck.')
    ...
    mongla.vision.align(gate, yaw=0, lat=0, err=ALIGN_ERR_PX, gain=ALIGN_GAIN,
    ...
    mongla.move_forward_dist(GATE_PASS_M, gain=GATE_PASS_GAIN)

    # ── Phase 4: HomeFlare ──────────────────────────────────────────────────
    log('Phase 4: homing on flare')
    mongla.vision.align(flare, yaw=0, depth=0, err=ALIGN_ERR_PX, gain=ALIGN_GAIN,
    ...
    mongla.move_forward_dist(RETURN_PASS_M, gain=RETURN_PASS_GAIN)
