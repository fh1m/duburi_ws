...
Run it:
    ...
    ros2 run mongla_planner mission sauvc_navigation
...
def run(mongla, log=None):
    ...
    try:
        ...
        if mongla.backend != 'srot':
            mongla.lock_heading(0.0, timeout=SAUVC_NAV_BUDGET_S)
        navigate(mongla, log)
    finally:
...
def navigate(mongla, log=None):
    ...
    mongla.set_classes(_GATE, node=_FWD)
    ...
    mongla.set_depth(SAUVC_GATE_PASS_DEPTH_M, timeout=20)
    ...
    if SAUVC_BLIND_TRANSIT_ENABLED and not passed:
        ...
        mongla.move_forward(SAUVC_BLIND_TRANSIT_S, gain=SAUVC_BLIND_GAIN)
