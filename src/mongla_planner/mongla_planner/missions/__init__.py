...
and it will show up in `ros2 run mongla_planner mission --list` on the
...
source is loaded immediately -- no `colcon build` or `source` required.
  ...
  * Override the source dir with ``MONGLA_MISSIONS_DIR=/path/to/missions``.
...
(``mongla_dsl.py``, ``client.py``, etc.) still require a rebuild because
...
def _warn_bad_mission(name: str, exc: Exception) -> None:
    ...
    if os.environ.get('MONGLA_MISSIONS_VERBOSE'):
        traceback.print_exc()
...
def _find_src_missions_dir() -> Path | None:
    ...
    Priority:
      1. ``MONGLA_MISSIONS_DIR`` environment variable (explicit override)
    ...
    """
    env = os.environ.get('MONGLA_MISSIONS_DIR')
    if env:
        p = Path(env)
        return p if p.is_dir() else None

    # Installed path looks like:
    #   <ws>/install/mongla_planner/lib/python3.x/site-packages/mongla_planner/missions/__init__.py
    this = Path(__file__).resolve()
    path_str = str(this)
    if '/install/' in path_str:
        ws_root = Path(path_str.split('/install/')[0])
        src_dir = (ws_root / 'src' / 'mongla_planner'
                   / 'mongla_planner' / 'missions')
        if src_dir.is_dir():
            return src_dir
...
def discover() -> Dict[str, Callable]:
    ...
    ``MONGLA_MISSIONS_VERBOSE=1`` for the full traceback.
        ...
        for py_file in sorted(src_dir.glob('*.py')):
            ...
            try:
                spec = importlib.util.spec_from_file_location(
                    f'mongla_planner.missions.{name}', py_file)
