...
Run: ros2 run mongla_planner mission heading_lock_demo
"""
...
def run(mongla, log):
    ...
    mongla.set_depth(0.0, timeout=40.0)
    ...
    mongla.lock_heading(0.0, timeout=LOCK_TIMEOUT_S)
    ...
    for leg in range(4):
        ...
        mongla.move_forward(LEG_DURATION_S, gain=LEG_GAIN_PCT)
        ...
        # correction rate via ros2 param set /mongla_manager vision.kp_yaw.
        if leg % 2 == 0:
            mongla.move_right(LEG_DURATION_S, gain=LEG_GAIN_PCT)
        else:
            mongla.move_left(LEG_DURATION_S, gain=LEG_GAIN_PCT)
