...
def test_the_package_is_in_the_build_script():
    ...
    build = (ROOT / 'build_mongla.sh').read_text()
    assert 'mongla_localization' in build
...
def test_prioritised_allocation_reaches_the_srot_frame():
    ...
    import sys
    sys.path.insert(0, str(ROOT / 'src' / 'mongla_control'))
    from mongla_control.motion_vision import _srot_drive
