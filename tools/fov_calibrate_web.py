#!/usr/bin/env python3
"""Shim. The real thing is `mongla_vision.calibration.guide`.
...
`from mongla_vision.calibration.guide import main` after putting the source
...
_M = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'src',
                  'mongla_vision', 'mongla_vision', 'calibration', 'guide.py')
_spec = importlib.util.spec_from_file_location('mongla_calib_guide',
                                               os.path.abspath(_M))
_mod = importlib.util.module_from_spec(_spec)
sys.modules['mongla_calib_guide'] = _mod
