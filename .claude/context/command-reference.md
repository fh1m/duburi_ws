...
| `gain_lat` / `gain_yaw` | float (%) | `0.0` | `0 – 100` | per-axis speed cap; **`0` = inherit `gain`** (NOT disable — omit the axis to drop it). **Depth has no % cap** — its rate is set by `depth_step` (m/update). |
...
| `settle_px` | float (px) | `0.0` | `≥ 0` | **precision (`settle`):** settle gate — only count a tick toward `align_stable_frames` when the hull is in-band AND barely moving (`\|Δerr\| ≤ settle_px`), so align ends SETTLED not mid-pass. Keyed on error velocity (a steady current doesn't block it — that's `ki_lat`'s job). **Per-call only, coarse aligns only — never the fire-lock** (it gates the mid-hold fire). `0` = off |
| `depth_step` | float (m) | `0.0`→`0.02` | `0.02 – 0.10` | **per-update depth-setpoint resolution.** Depth steps the ALT_HOLD setpoint by ≤ this each 5 Hz update and **freezes inside the deadband**, so ArduSub settles (no z-wobble). The SOLE depth-rate knob (0.02 slow .. 0.10 coarse). `0` = default (0.02) via the verb; drop the depth axis by omitting `depth` from `axes` |
| `fire_pass_enabled` | bool | `false` | `true`/`false` | **fire even if never fully aligned:** fire the payload at a natural exit (TIMEOUT / hold-complete) provided the target was seen LIVE within `lost_grace_s` — a guaranteed partial-points shot. Never fires on a never-seen or coasted-only target. Off = fire only on a strict in-band lock |
| `hold_heading` | bool | `false` | `true`/`false` | **fire-window quiet mode:** when yaw is released to the background heading lock, widen the lock deadband (1°→3°) for the hold so the launcher heading holds STEADY instead of micro-correcting sub-deg noise (the terminal yaw jitter). Restored on exit |
...
| Aspect | Value |
|---|---|
| CLI | `duburi vision_align --camera forward --target_class hole --axes lat,depth [--offset_lat 0] [--err_px 14] [--duration 25] [--gain 12] [--brake_off] [--hold_s 4] [--fire_channels 1] [--fire_t 1.5] [--fwd_fill 35] [--mode height] [--depth_step 0.05] [--fire_pass_enabled] [--hold_heading]` (`--fwd_fill`>0 = forward range-hold standoff shot) |
| DSL | `duburi.vision.align('hole', lat=0, depth=0, fwd=None, fwd_mode='area', err=40, duration=20, gain=30, lat_gain=None, yaw_gain=None, depth_step=None, brake=True, brake_gain=None, hold=None, fire=None, fire_t=None, lock_on=False, settle=None, fire_pass=False, hold_heading=False, fallback=None, camera=None)` (no `depth_gain` — depth rate is `depth_step`) |
| Facade | `Duburi.vision_align(camera, target_class, axes, offset_lat=, offset_yaw=, offset_depth=, err_px=, duration=, gain=, gain_lat=, gain_yaw=, brake_off=, brake_gain=, hold_s=, hold_through_loss=, fire_channels=, fire_t=, kp_lat=, kp_yaw=, kp_depth=, lost_grace_s=, align_stable_frames=, lock_target=, ctrl_conf=, range_gain_floor=, ki_lat=, coast_s=, fwd_fill=, mode=, kp_forward=, settle_px=, depth_step=, fire_pass_enabled=, hold_heading=)` |
| Forward range-hold (standoff shot) | `fwd_fill`>0 (DSL `fwd=`, % fill) + `mode` (DSL `fwd_mode=`area/width/height) adds a forward axis: align drives forward to that standoff fill and HOLDS it while centering lat/depth → ONE verb does the full torpedo standoff shot (forward-standoff + lat/depth + hold + mid-hold fire). One-sided forward (never reverses, no reverse-kick/ramming); the mid-hold fire is gated on reaching the standoff too. `fwd_fill`=0 (default) = no forward axis. |
...
| Per-axis gain | `lat_gain`/`yaw_gain` cap the lat/yaw axis; **unset/0 = inherit `gain`, NOT disable** (omit `lat`/`yaw`/`depth` to drop an axis). **Depth has no % cap** — its rate is `depth_step` (m/update). The yaw spin-up floor only engages close-up (large bbox ≥ `VISION_YAW_FLOOR_FILL`) and is now **tapered** to 0 at the deadband edge (mirrors `heading_lock`/`motion_yaw`) so it eases in instead of relay-slamming; far-field yaw is pure-proportional so it cannot limit-cycle/wobble. |
| Depth (stepped, no wobble) | The depth axis nudges an ArduSub **ALT_HOLD setpoint** (not raw RC). It steps the setpoint by ≤ `depth_step` m **only in the 5 Hz block** and **freezes inside the deadband**, so ArduSub's depth PID settles between steps (max slew = `depth_step × 5 Hz`). `depth_step` (DSL, m) is the sole depth-rate knob — 0.02 slow, 0.10 coarse. This is the z-wobble fix; see [`BUGS.md`](BUGS.md) D12. |
| Fire freshness (`is_new_frame`) | The mid-hold fire leaves **only on the tick a genuinely new, non-coasted detection lands** — FPS-robust (fires promptly at low detector rate) yet **never** on a frozen detector's re-read stale frame or a Kalman-coasted box, even while `stable` stands held at threshold through a mid-hold freeze. `fire_pass=True` adds a guaranteed end-of-command shot if the strict lock never fired. See [`BUGS.md`](BUGS.md) D11/D12. |
| Terminal yaw hold (`hold_heading`) | At the hole the mission **drops the yaw axis** (Ch4 → `heading_lock`). `hold_heading=True` widens the lock deadband 1°→3° for the hold so the launcher heading holds steady (no terminal yaw jitter). Pair it with the yaw-dropped fire-lock. |
| **Downward frame** (`camera=downward`/`sim_bottom`) | The bottom cam hovers over the target, so the body axes rotate: **image-X→Ch6 lateral**, **image-Y→Ch5 SURGE fore/aft** (the `depth` axis; two-sided P + arrival brake), and **`fwd_fill`→DEPTH descent** = the "approach/get-closer" (driven by the `depth_step` proportional/deadband-frozen logic, one-sided deeper). `surge_sign` (+1/-1) flips fore/aft for the mount — **verify DISARMED** (`vision_thrust_check --camera downward`); a wrong sign is positive feedback. Bounded deep by `max_depth_m` (floor) and shallow by `depth_ceiling_m`. **What actually keeps the downward hull from surfacing is structural** — ArduSub holds `set_depth` on Ch3 and the vision loop only drives Ch5/Ch6 (horizontal), and the descent is one-sided-deeper; `depth_ceiling_m` is belt-and-suspenders that only binds if `fwd_fill`→depth descent is enabled (or on the forward depth axis). `fire` drops droppers (3/4). `move(camera='downward')` is rejected (surge doesn't grow fill). See [`dual-camera-setup.md`](dual-camera-setup.md), `task_bin.py`. |
| Multi-shot gap (`fire_gap`) | Firing a **list** (`fire=[1,4]`/`[2,3]`) spaces the channels `fire_gap` s apart (default via `FIRE_GAP_S=1.0`); the solenoid launcher misfires if two go together. Single-channel fires are unaffected. Abort-interruptible during the gap. |
