...
| 🟠 | **Stray `missions/mission.py`** is a stale, fully-commented-out copy of `pursue_demo` (its docstring says "pursue_demo"). `discover()` registers it as a phantom mission named **`mission`**, and it carries a `lock_heading(degrees=0.0)` call whose kwarg may not match the DSL. Confusing duplicate of both `pursue_demo.py` and the real runner `duburi_planner/mission.py`. | `missions/mission.py` | Delete it. `pursue_demo.py` is the live version. |
| 🟠 | **Missions have no `try/finally` disarm.** `gate_flare_autonomous` only calls `_surface_and_disarm` on its own success/early-return paths; an exception inside `vision.home`/`move_forward_dist` exits the mission with the sub **armed and station-keeping** (heartbeat + heading-lock keep it failsafe-quiet). Relies on operator Ctrl-C → manager `_emergency_stop`. | `gate_flare_autonomous.py:61`, others | Wrap mission bodies in `try/finally: _surface_and_disarm`. Manager-side emergency stop is the backstop, not the primary. |
| 🟡 | **`vision.use_tracks` param has a lasting side-effect.** A single goal with `tracking=True` calls `set_parameters(vision.use_tracks=True)` and never restores it — every subsequent new-camera `VisionState` then defaults to tracks. The `(camera,use_tracks)` cache key prevents poisoning *existing* states but the node param stays flipped. | `auv_manager_node.py:533` | Snapshot+restore the param around the goal, or pass `use_tracks` per-build without mutating the node param. |
| 🔵 | `pursue_demo` (the keeper) calls `duburi.vision.find(move='yaw_right')` / `vision.turn()` without a `target=` — relies on sticky `duburi.target`. Confirm sticky-context default is set (it is via `duburi.target = TARGET_CLASS`). | `pursue_demo.py:43` | OK; note for mission authors. |
...
- **Sign conventions**: documented (Ch4>1500=yaw LEFT, Ch6>1500=strafe RIGHT) and recently corrected (`1801fe2`). Cross-check against `feedback_yaw_direction` memory before pool day.
...
- 🔵 No `SET_MESSAGE_INTERVAL` for SYS_STATUS; battery comes from BATTERY_STATUS@1 Hz — fine. POSHOLD-dependent `look_around` needs the Nortek BlueOS extension + EK3 params (documented in `dvl-reference.md`); verify on the actual vehicle.
...
- **Control loop verified by reading** (`motion_vision.py:335-418`): single RC packet carries Ch3/4/5/6; per-axis deadband; `gate_guard`, `pass_at`, `pursue`-clamp, `depth_anchor_frac` all implemented cleanly. **Sign conventions are correct**: yaw negates (`-ex*kp_yaw`, since Ch4>1500=yaw LEFT → `:341`), lateral does not (`+ex*kp_lat`, Ch6>1500=strafe RIGHT → `:349`) — matches the `1801fe2` fix and CLAUDE.md §6. **Verdict: well-built.**
- 🔵 **Stale module docstring in `motion_vision.py:42`** shows the *pre-fix* un-negated `yaw_pct = clamp(ex * Kp_yaw)`; the live code at `:341` negates. A tuner reading the header would get the yaw sign backwards. Fix the docstring to match `:341`.
...
- 🟡 No tests for `auv_manager_node` dispatch/abort, ~~`connection_config.resolve_mode`~~, DVL parser (`nucleus_parser`). **`resolve_mode`/`resolve_profile` now covered** — `src/duburi_manager/test/test_connection_config.py` (16 tests, probes monkeypatched). Remaining: manager dispatch/abort, `nucleus_parser`.
...
> - 🟡 §3.1 `vision.use_tracks` permanent flip — `auv_manager_node.py:533` now snapshots + restores the param around the goal (try/finally), so a `tracking=True` goal no longer poisons later goals' default. (Relies on VisionState being built synchronously inside the goal call — true per the one-shot `wait_vision_state_ready` preflight.)
> - 🔵 §3.6 stale `motion_vision.py:42` docstring — yaw formula corrected to the negated (Ch4-inverted) live form; yaw mapping extracted to pure `_yaw_pct()` and **unit-tested** (the sign that bit in `1801fe2`).
>
