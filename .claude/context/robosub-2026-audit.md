...
- **Pipeline**: camera_node → detector_node → tracker_node (Roboflow OC-SORT/ByteTrack via the `Tracker` ABC + Kalman, registry pruning) → depth_estimation_node (`vis_range`, Depth Anything V2-Small ONNX + bbox-area fallback). HUD `vision_display`. Closed loop runs INSIDE the manager (`motion_vision`) so vision and control never fight for thrust; `/tracks` is display-only unless `vision.coast_s>0` (opt-in gap-bridging coast).
