...
- **Control loops verified by reading** (`motion_vision.py`, `align_loop` + `move_loop`): per-tick P-on-pixel error with `gain` as a hard max-speed clamp; `align_loop` centres on lat/yaw/depth (each at a signed pixel offset), `move_loop` drives forward to a bbox fill ratio. **Sign conventions are correct**: yaw negates (`-ctrl*kp_yaw`, since Ch4>1500=yaw LEFT), lateral does not (`+ctrl*kp_lat`, Ch6>1500=strafe RIGHT) — matches the `1801fe2` fix and CLAUDE.md §6. **Verdict: well-built.**
