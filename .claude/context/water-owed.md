...
| 24 | **re-derive the de-rotation gains on THIS mount** — measured three ways on the vehicle (`measured-bars.md` §19): shipped `(-1,-1)` 109.4 %, **off `(0,0)` 105.0 % and 3x tighter**, flipped `(+1,+1)` 94.6 %. The true gain is inside (−1,+1) and near zero. §12 predicted this and it was never done. `tools/flow_derot_calibrate.py`, tilt-only, one axis per run. **Needs no water** | §19 |
| 25 | **A/B `reanchor_on_refusal`** — a refused interval used to discard the travel it covered, and refusals cluster in the FAST part of a move. Fixed behind a parameter, default unchanged; its effect has not yet been measured on a working rig | §18 |
