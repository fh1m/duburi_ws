# duburi_manager — the node that owns the vehicle

**8.5k lines · 388 tests · [`src/duburi_manager`](../../../../src/duburi_manager)**
...
```
duburi arm ─────► /duburi/move (action) ─────► auv_manager_node
                                                    │
                                    ┌───────────────┼────────────────┐
                                    ▼               ▼                ▼
...
| Topic | What | Notes |
|---|---|---|
| `/duburi/state` | armed, mode, heading, depth, battery | on change plus a heartbeat; a missing number is `NaN`, never `0.0` |
| `/duburi/imu` · `/duburi/imu_rates` | the board's inertial data | stamped with the board's own clock where available |
| `/duburi/demand` | what we just asked for | the estimator uses it to predict motion |
| `/duburi/esc_rpm` | per-thruster RPM | srot only — it did not exist before |
| `/duburi/flare_order` | the competition order received over the radio | srot only, latched |

## Entry points

| Command | What it does |
|---|---|
| `ros2 run duburi_manager start` | the node itself (also `auv_manager`, `auv_manager_node`) |
| `ros2 run duburi_manager connect` | open the board's link and print everything it sends — no ROS graph, no built workspace needed. `--watch` for live, `--json` for machines |
| `ros2 run duburi_manager bringup_check` | preflight that grades each subsystem and exits non-zero on a real fault; `--srot` adds the board-specific checks |
| `ros2 run duburi_manager autotune` | run the board's own gain tuning, in water, behind a typed confirmation |
| `ros2 run duburi_manager flare_order` | send the competition's flare order to the vehicle over the radio |
...
```bash
python3 -m pytest -q src/duburi_manager/test
```
...
Related: [`duburi_control`](../duburi_control/README.md) (what the verbs actually do) ·
[`duburi_interfaces`](../duburi_interfaces/README.md) (the action) ·
