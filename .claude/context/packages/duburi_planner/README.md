# duburi_planner — how you tell the vehicle what to do

**11k lines · 405 tests · [`src/duburi_planner`](../../../../src/duburi_planner)**
...
The CLI is *generated* from the command registry in `duburi_control`. Add a verb there and it
...
A mission is a Python file with a `run(duburi)` function. The object it is handed is the whole
vehicle:
...
def run(duburi, log=None):
    ...
    if duburi.detected('gate'):
        duburi.vision.align('gate', yaw=0, lat=0, gain=30)
...
**A task can be abandoned cleanly.** `with duburi.task('gate', deadline_s=90):` cancels the
...
| File | Role |
|---|---|
| `duburi_dsl.py` | the mission language |
| `vision_dsl.py` | `duburi.vision.align` and `.move`, and their results |
| `client.py` | the blocking action client: deadlines, cancellation, goal ids |
| `cli.py` | the `duburi` command, generated from the registry |
...
```bash
python3 -m pytest -q src/duburi_planner/test
```
...
Related: [`duburi_control`](../duburi_control/README.md) (the verbs) ·
[`duburi_manager`](../duburi_manager/README.md) (what serves them) ·
