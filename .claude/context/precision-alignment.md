...
> arm it; (3) **freshness** — the sample must be a live detection (`not coasted`) no
> older than `VISION_FRESH_FULL_S`. So a torpedo **never** fires on a tracker-coasted
> (Kalman-predicted) box during a `coast_s` gap, nor on a frozen detector's stale
> cached frame. Coast exists to *hold the lock*, not to *take the shot*. If the hole
> detection is dropping at the standoff, the shot waits for it to come back (within
> the hold window) rather than firing blind. This is why raising real detector FPS
> (`[YOLO] backend=TensorRT`) is the single biggest lever on fire reliability.
