...
| Verb | What it does |
|------|--------------|
| `vision.align(target, *, lat=, yaw=, depth=, fwd=, fwd_mode=, hold=, lock_on=, settle=, ...)` | Centre `target` on the named axes, each at a signed **pixel offset** from frame centre. `hold=`s = **active station-keep**: keep correcting on-target for `hold` s before exiting (holds the hull steady for a torpedo/dropper shot); counts against `duration`. **`fwd=`** (% fill, `fwd_mode=`area/width/height) adds a **forward range-hold axis** — align ALSO drives forward to that standoff and holds it, so one verb does forward-standoff + lat/depth + hold + mid-hold fire (the torpedo standoff shot; one-sided forward, never reverses; fire gated on the standoff too). `fwd` unset = lat/yaw/depth only. `lock_on=True` = precision continuity lock. **`settle=`**`<px>` = settle gate (ends settled like `vision.move`; coarse aligns only, never the fire-lock). |
